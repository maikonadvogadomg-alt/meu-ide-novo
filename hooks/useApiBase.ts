 ...
 * Prioridade: EXPO_PUBLIC_API_BASE_URL > customServerUrl (settings) > auto-detect (Termux â Cloud)
 * A porta do Termux Ã© configurÃ¡vel em ConfiguraÃ§Ãµes > Porta Termux (padrÃ£o: 8080).
 * Retorna string vazia quando nÃ£o hÃ¡ API disponÃ­vel (modo offline) OU quando serverEnabled=false.
 * Verifica a cada 8s â se o Termux for iniciado depois, detecta automaticamente.
 */
export function useApiBase(): string {
  const { settings } = useApp();

  // Se o servidor estiver desabilitado nas configuraÃ§Ãµes, retorna vazio imediatamente.
  ...
  useEffect(() => {
    // Servidor desligado pelo usuÃ¡rio â funciona sÃ³ com chaves diretas de IA
    if (!serverEnabled) {
      setActive("");
