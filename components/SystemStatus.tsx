...
export default function SystemStatus({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const runAllChecks = useCallback(async () => {
    ...
    const initial: Check[] = [
      { id: "internet", label: "ConexÃ£o Ã  internet", icon: "wifi", stat: "checking", detail: "Verificando..." },
      ...
      { id: "disk",     label: "EspaÃ§o em disco", icon: "hard-drive", stat: "checking", detail: "Verificando..." },
      { id: "mem",      label: "MemÃ³ria RAM", icon: "activity", stat: "checking", detail: "Verificando..." },
      { id: "ai",       label: "IA Cortesia (Gemini)", icon: "zap", stat: "checking", detail: "Verificando..." },
      { id: "vscode",   label: "VS Code (vscode.dev)", icon: "monitor", stat: "idle", detail: "AcessÃ­vel via aba Plugins" },
    ...
    try {
      ...
      update("internet", { stat: r ? "ok" : "fail", detail: r ? "Online â" : "Offline â sem internet â" });
    } catch {
      update("internet", { stat: "fail", detail: "Offline â" });
    }

    // 2 â API server (tenta 3x antes de marcar como falha)
    if (!API_BASE) {
      update("api", { stat: "warn", detail: "Servidor nÃ£o configurado. Configure nas ConfiguraÃ§Ãµes." });
    } else {
      let ok = false;
      ...
      update("api", { stat: ok ? "ok" : "warn", detail: ok ? `Servidor online â` : `Servidor nÃ£o respondeu â verifique conexÃ£o` });
    }

    // 3 â Terminal (run a real command)
    {
      const out = await runCmd("echo 'DEVMOBILE_OK' && uname -srm", session, TERMINAL_API);
      const ok = out.includes("DEVMOBILE_OK");
      update("terminal", {
        stat: ok ? "ok" : "fail",
        detail: ok ? "Comandos Linux executam de verdade â" : "Falha na execuÃ§Ã£o â",
        realOutput: out,
      });
    }
    ...
    // 4 â Node.js
    {
      ...
      update("node", {
        stat: match ? "ok" : "fail",
        detail: match ? `VersÃ£o: ${match[0]} â` : "NÃ£o encontrado â",
        realOutput: out,
      });
    }
    ...
    // 5 â npm
    {
      ...
      update("npm", {
        stat: match ? "ok" : "warn",
        detail: match ? `npm v${match[0]} â` : "NÃ£o encontrado",
        realOutput: out,
      });
    }
    ...
    // 6 â Python
    {
      ...
      update("python", {
        stat: match ? "ok" : "warn",
        detail: match ? `${match[0]} â` : "NÃ£o encontrado",
        realOutput: out,
      });
    }
    ...
    // 7 â pip
    {
      ...
      update("pip", {
        stat: hasPip ? "ok" : "warn",
        detail: hasPip ? `${out.split(" from")[0]} â` : "NÃ£o encontrado",
        realOutput: out,
      });
    }
    ...
    // 9 â Bash
    {
      ...
      update("bash", {
        ...
        detail: hasBash ? `${out.slice(0, 50)} â` : "NÃ£o encontrado",
        realOutput: out,
      });
    }
    ...
    // 10 â Disk
    {
      ...
      update("disk", {
        ...
        detail: out.includes("%") ? `${out.replace(/\s+/g, " ").trim()}` : "NÃ£o foi possÃ­vel verificar",
        realOutput: out,
      });
    }
    ...
    // 11 â Memory
    {
      ...
      update("mem", {
        stat: hasMem ? "ok" : "warn",
        detail: hasMem ? out.replace(/\n/g, " | ").trim() : "NÃ£o foi possÃ­vel verificar",
        realOutput: out,
      });
    }

    // 12 â AI (Gemini Direto tem prioridade, depois tenta servidor)
    if (hasDirectKey) {
      try {
        ...
        update("ai", {
          stat: r?.ok ? "ok" : "warn",
          detail: r?.ok ? "Gemini DIRETO do celular â (sem servidor!)" : `Chave invÃ¡lida ou offline (${r?.status})`,
        });
      } catch {
      ...
      try {
        const r = await fetch(`${API_BASE}/ai/chat`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ messages: [{ role: "user", content: "responda sÃ³: OK" }] }),
          cache: "no-store",
        }).catch(() => null);
        update("ai", {
          stat: r?.ok ? "ok" : "warn",
          detail: r?.ok ? "Gemini via servidor â" : "Servidor offline â adicione chave Gemini nas ConfiguraÃ§Ãµes",
        });
      } catch {
        update("ai", { stat: "warn", detail: "Sem acesso ao Gemini â adicione chave nas ConfiguraÃ§Ãµes" });
      }
    } else {
      update("ai", { stat: "warn", detail: "Sem Gemini â configure chave em ConfiguraÃ§Ãµes â GEMINI DIRETO" });
    }

    // 13 â VS Code (static)
    update("vscode", {
      stat: "ok",
      detail: "vscode.dev disponÃ­vel (WebView) â abre via Plugins â",
    });
    ...
    update("projects", {
      ...
      detail: activeProject
        ? `Projeto ativo: "${activeProject.name}" Â· ${activeProject.files.length} arquivo(s) â`
    ...
    update("git_acc", {
      ...
      detail: gitConfigs.length > 0
        ? `Conectado: ${gitConfigs.map(g => `${g.provider} (${g.username})`).join(", ")} â`
        : "Nenhuma conta Git â configure em Projetos â GitHub",
    });
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <View style={{ flex: 1, backgroundColor: "#0a0a0a" }}>
        ...
        <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>ð©º STATUS DO SISTEMA â REAL</Text>
            <Text style={styles.subtitle}>VerificaÃ§Ãµes ao vivo no servidor Linux</Text>
          </View>
        ...
        {done && (
          <View style={styles.summaryBar}>
            <Text style={{ color: "#00d4aa", fontSize: 13, fontWeight: "700" }}>â {ok} OK</Text>
            <Text style={{ color: "#f59e0b", fontSize: 13, fontWeight: "700" }}>â ï¸ {warn} atenÃ§Ã£o</Text>
            <Text style={{ color: "#ef4444", fontSize: 13, fontWeight: "700" }}>â {fail} falha</Text>
          </View>
        )}
        {running && (
          ...
          {done && (
            <View style={{ marginTop: 20, backgroundColor: "#0d1f0d", borderWidth: 1, borderColor: "#1f4d1f", borderRadius: 12, padding: 14 }}>
              <Text style={{ color: "#4ade80", fontWeight: "800", fontSize: 14, marginBottom: 10 }}>
                â O QUE FUNCIONA DE VERDADE
              </Text>
              {[
                "Terminal Linux â bash, node, python3, git, pip, npm, curl, wget, tudo",
                "npm install QUALQUER pacote â output completo linha por linha",
                "pip3 install QUALQUER biblioteca Python",
                "VS Code real â vscode.dev â igual no desktop (aba Plugins)",
                "SincronizaÃ§Ã£o bidirecional: app â VS Code â app (Upload/Download)",
                "Git + GitHub â clone, push, pull, status, diff",
                "IA Jasmim â sem precisar de API key (cortesia Gemini)",
                "Campo Livre â chat sem restriÃ§Ãµes, com TTS e seleÃ§Ã£o de voz",
                "3 terminais simultÃ¢neos com sessÃµes independentes",
                "Instalar Python, Node.js, TypeScript, Rust, Go, Java... pela aba Plugins",
                "Exportar/Importar projeto como ZIP",
                "Playground HTML â visualizar HTML/CSS/JS na hora",
                "Banco de dados Postgres (Neon) via configuraÃ§Ãµes",
              ].map((item, i) => (
                <View key={i} style={{ flexDirection: "row", gap: 8, marginBottom: 6 }}>
                  <Text style={{ color: "#4ade80", fontSize: 13 }}>âº</Text>
              ...
              {(hasDirectKey ? [
                "Gemini IA: sÃ³ precisa de internet (chave direta configurada â)",
                "Terminal Linux: precisa do servidor (Termux no celular)",
                "Termux + Termux:API instalado = terminal funciona 100% offline no celular",
              ] : [
                "Gemini IA: precisa do servidor externo OU adicione sua chave em ConfiguraÃ§Ãµes",
