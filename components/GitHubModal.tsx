...
export default function GitHubModal({
  ...
  useEffect(() => {
    if (visible) {
      ...
      setToken(t);
      // Se recebeu initialView, usa ela; senÃ£o comportamento padrÃ£o
      if (initialView) {
  ...
  const handleSaveToken = async () => {
    ...
    try {
      ...
      Alert.alert("â Token invÃ¡lido", e?.message || "Verifique o token e tente novamente.");
    }
    setLoadingUser(false);
  };
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              setCloneProgress(null);
              Alert.alert("â Erro", e?.message || String(e));
            }
            setCloning(false);
          },
        },
  ...
  const handleCreateAndPush = async () => {
    ...
    try {
      ...
      setCreateResult({
        ok: true,
        msg: `â Enviado! ${result.pushed} arquivo(s) em github.com/${owner}/${createName.trim()}`,
      });
    } catch (e: any) {
  ...
  const handlePushExisting = async () => {
    ...
    try {
      ...
      const result = await pushFiles(
        ...
        pushRepo.trim(),
        files,
        pushMsg.trim() || "DevMobile â atualizaÃ§Ã£o"
      );
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      setLastPushedRepo({ owner: pushOwner.trim(), repo: pushRepo.trim() });
      setPagesInfo(null);
      setPushResult({ ok: true, msg: `â Enviado! ${result.pushed}/${result.total} arquivo(s) enviados.` });
    } catch (e: any) {
  ...
  const handlePublishPages = async (repoOverride?: { owner: string; repo: string }) => {
    ...
    if (!target) {
      Alert.alert("Aviso", "Envie o projeto para um repositÃ³rio GitHub primeiro.");
      return;
    }
    ...
    try {
      ...
      log("ð¨ Build concluÃ­do! Lendo arquivos...");
      await new Promise(r => setTimeout(r, 400));
      log(`ð¦ ${fileCount} arquivo(s) prontos. Verificando repositÃ³rio GitHub...`);
      await new Promise(r => setTimeout(r, 500));
      log(`ð RepositÃ³rio ${owner}/${repo} encontrado.`);
      await new Promise(r => setTimeout(r, 300));
      log(`ð Enviando para o GitHub...`);

      // Garante repositÃ³rio pÃºblico (GitHub Pages grÃ¡tis exige repo pÃºblico)
      try { await makeRepoPublic(token, owner, repo); } catch {}

      log("ð Habilitando GitHub Pages...");
      const info = await enablePages(token, owner, repo, "main", "/");
      setPagesInfo(info);

      log(`â Pronto! GitHub Pages habilitado.`);
      log(`ð Publicado em ${info.url}`);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e: any) {
      log(`â Erro: ${e?.message || String(e)}`);
      Alert.alert(
        "â GitHub Pages",
        `${e?.message || String(e)}\n\nDica: o repositÃ³rio precisa ser pÃºblico para usar GitHub Pages gratuito.`
      );
    }
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose} statusBarTranslucent>
      <View style={[s.container, { backgroundColor: colors.background }]}>
        ...
        {view === "main" && (
          <ScrollView contentContainerStyle={s.body}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[s.actionLabel, { color: "#a78bfa" }]}>Publicar no GitHub Pages â GrÃ¡tis</Text>
                <Text style={[s.actionDesc, { color: colors.mutedForeground }]}>Editor de cÃ³digo completo, sem mensalidade, para sempre</Text>
              </View>
            ...
            {showPagesMain && (
              <View style={{ gap: 8, marginBottom: 8 }}>
                <Text style={{ color: colors.mutedForeground, fontSize: 12, lineHeight: 17 }}>
                  Compila este editor e publica ele no GitHub Pages â um endereÃ§o tipo{" "}
                  ...
                  EdiÃ§Ã£o de arquivos e integraÃ§Ã£o com GitHub funcionam sem o servidor. SÃ³ a IA e o terminal online precisam do servidor.
                </Text>
                <TextInput
                  style={[s.input, { color: colors.foreground, borderColor: "#7c3aed88", backgroundColor: colors.card }]}
                  placeholder="Nome do repositÃ³rio (ex: sk-editor)"
                  placeholderTextColor={colors.mutedForeground}
                  value={createName}
                ...
                >
                  ...
                  <Text style={[s.btnText, { color: "#fff" }]}>
                    {pagesLoading ? "Publicando... (pode demorar 2 min)" : "ð Publicar no GitHub Pages"}
                  </Text>
                </TouchableOpacity>
                {pagesLogs.length > 0 && (
                ...
                {pagesInfo && (
                  <View style={{ borderRadius: 10, padding: 14, backgroundColor: "#22c55e14", borderWidth: 1, borderColor: "#22c55e44", gap: 8 }}>
                    <Text style={{ color: "#22c55e", fontWeight: "800", fontSize: 14 }}>â Publicado com sucesso!</Text>
        ...
        {view === "import" && (
          <View style={{ flex: 1 }}>
            {/* URL pÃºblica rÃ¡pida no topo */}
            <View style={{ paddingHorizontal: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.border, gap: 6 }}>
              <Text style={{ color: colors.mutedForeground, fontSize: 11, fontWeight: "700", textTransform: "uppercase" }}>Importar por URL pÃºblica</Text>
              ...
              <Text style={{ color: colors.mutedForeground, fontSize: 10 }}>Funciona sem token para repositÃ³rios pÃºblicos. Ou selecione da sua lista abaixo:</Text>
            </View>
            ...
            <View style={[s.searchBar, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
              ...
              <TextInput
                ...
                placeholder="Buscar nos seus repositÃ³rios..."
                placeholderTextColor={colors.mutedForeground}
                autoCapitalize="none"
              />
            ...
            {cloning ? (
              <View style={{ flex: 1, alignItems: "center", justifyContent: "center", gap: 16, paddingHorizontal: 24 }}>
                ...
                <Text style={{ color: colors.mutedForeground, fontSize: 12, textAlign: "center" }}>
                  {"RepositÃ³rios grandes podem levar alguns segundos.\nMantenha o app aberto."}
                </Text>
              </View>
            ) : (
              <FlatList
            ...
            {createResult && (
              <View style={{ gap: 8 }}>
                ...
                {createResult.ok && lastPushedRepo && (
                  <View style={{ gap: 8 }}>
                    {/* Abrir no editor externo â VS Code / StackBlitz / Gitpod */}
                    <Text style={{ color: "#6b7280", fontSize: 11, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.5 }}>
                      Abrir no Editor
                    </Text>
                    {[
                      { label: "VS Code Web", desc: "Editor completo, grÃ¡tis", url: `https://github.dev/${ghUser?.login || ""}/${createName.trim()}`, color: "#007acc", emoji: "ð" },
                      { label: "StackBlitz", desc: "VS Code + Node.js + npm, grÃ¡tis", url: `https://stackblitz.com/github/${ghUser?.login || ""}/${createName.trim()}`, color: "#1389fd", emoji: "â¡" },
                      { label: "Gitpod", desc: "Terminal Linux completo, 50h grÃ¡tis/mÃªs", url: `https://gitpod.io/#https://github.com/${ghUser?.login || ""}/${createName.trim()}`, color: "#ff8a00", emoji: "ð " },
                    ].map((item) => (
                      <TouchableOpacity
                        key={item.label}
                    ...
                    <Text style={{ color: "#6b7280", fontSize: 10, paddingHorizontal: 4 }}>
                      Edite lÃ¡ e use "Importar repositÃ³rio" para trazer de volta.
                    </Text>
                    <TouchableOpacity
                    ...
                    >
                      {pagesLoading ? <ActivityIndicator size="small" color="#fff" /> : <Feather name="globe" size={16} color="#fff" />}
                      <Text style={[s.btnText, { color: "#fff" }]}>{pagesLoading ? "Publicando... (pode demorar 2 min)" : "ð Publicar no GitHub Pages â GrÃ¡tis"}</Text>
                    </TouchableOpacity>
                    {/* Logs em tempo real */}
                    {pagesLogs.length > 0 && (
                      <View style={{ backgroundColor: "#0a0a0a", borderRadius: 10, padding: 12, borderWidth: 1, borderColor: "#1a1a2e", gap: 4 }}>
        ...
        {view === "push-existing" && (
          <ScrollView contentContainerStyle={s.body}>
              ...
              <View>
                ...
                {pushResult.ok && pushOwner.trim() && pushRepo.trim() && (
                  <View style={{ marginTop: 10, gap: 8 }}>
                    ...
                    {[
                      { label: "VS Code Web", desc: "Editor completo, grÃ¡tis", url: `https://github.dev/${pushOwner.trim()}/${pushRepo.trim()}`, color: "#007acc", emoji: "ð" },
                      { label: "StackBlitz", desc: "VS Code + Node.js + npm, grÃ¡tis", url: `https://stackblitz.com/github/${pushOwner.trim()}/${pushRepo.trim()}`, color: "#1389fd", emoji: "â¡" },
                      { label: "Gitpod", desc: "Terminal Linux completo, 50h grÃ¡tis/mÃªs", url: `https://gitpod.io/#https://github.com/${pushOwner.trim()}/${pushRepo.trim()}`, color: "#ff8a00", emoji: "ð " },
                    ].map((item) => (
                      <TouchableOpacity
                        key={item.label}
                        onPress={() => Linking.openURL(item.url)}
            ...
            {/* GitHub Pages apÃ³s push existente */}
            {pushResult?.ok && lastPushedRepo && (
              <View style={{ gap: 8, marginTop: 4 }}>
                ...
                >
                  {pagesLoading ? <ActivityIndicator size="small" color="#fff" /> : <Feather name="globe" size={16} color="#fff" />}
                  <Text style={[s.btnText, { color: "#fff" }]}>{pagesLoading ? "Publicando..." : "ð Publicar no GitHub Pages â GrÃ¡tis"}</Text>
                </TouchableOpacity>
                {pagesLogs.length > 0 && (
                  <View style={{ backgroundColor: "#0a0a0a", borderRadius: 10, padding: 12, borderWidth: 1, borderColor: "#1a1a2e", gap: 4 }}>
                    {pagesLogs.map((l, i) => (
