...
const FILE_ICONS: Record<string, string> = {
  html: "ð", css: "ð¨", scss: "ð¨", js: "â¡", ts: "ð·",
  jsx: "âï¸", tsx: "âï¸", json: "ð", md: "ð", txt: "ð",
  py: "ð", go: "ð¹", rs: "ð¦", java: "â", kt: "ðª",
  swift: "ð", sh: "âï¸", yaml: "âï¸", yml: "âï¸", toml: "âï¸",
  env: "ð", lock: "ð", xml: "ð°", sql: "ðï¸",
};

function fileIcon(path: string) {
  const ext = path.split(".").pop()?.toLowerCase() ?? "";
  if (isBinaryPath(path)) return "ð";
...
function msgColor(msg: string) {
  if (msg.startsWith("â")) return GREEN;
  if (msg.startsWith("â")) return RED;
  return WHITE;
}
function msgBg(msg: string) {
  if (msg.startsWith("â")) return "#052e16";
  if (msg.startsWith("â")) return "#1c0505";
  return "#0a1628";
}
function msgBorder(msg: string) {
  if (msg.startsWith("â")) return "#166534";
  if (msg.startsWith("â")) return "#7f1d1d";
  return BORDER;
}
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      <View style={[st.sheetRoot, { paddingTop: insets.top + 8, backgroundColor: "#060e1c" }]}>
        <View style={st.sheetHeader}>
          <View style={{ flex: 1 }}>
            <Text style={st.sheetTitle} numberOfLines={1}>
              {file ? fileIcon(file.path) : "ð"} {file?.path.split("/").pop()}
            </Text>
        ...
        <View style={[st.sheetFooter, { paddingBottom: Math.max(insets.bottom, 12) }]}>
          <Text style={[st.hint, { color: MUTED }]}>{content.length.toLocaleString()} chars Â· {editing ? "modo ediÃ§Ã£o" : "somente leitura"}</Text>
        </View>
      </View>
    </Modal>
  );
...
function PushModal({
  ...
  const btnTxt = mode === "vscode" ? "Subir e abrir VS Code" : "Criar repositÃ³rio e enviar";

  async function doPush() {
    if (!repoName.trim()) { Alert.alert("Nome obrigatÃ³rio"); return; }
    if (!ghToken) { Alert.alert("Token GitHub necessÃ¡rio", "Configure na aba Importar."); return; }
    if (!files.length) { Alert.alert("Sem arquivos", "Importe um projeto primeiro."); return; }
    setPushing(true); setMsg("Criando repositÃ³rio..."); setDone(false);
    try {
      const repo = await ghCreateRepo(ghToken, repoName.trim(), `DevMobile APK Builder Â· ${source}`, isPrivate);
      ...
      setMsg(
        `â ${files.length} arquivos enviados!\n` +
        (pages ? `ð GitHub Pages: https://${owner}.github.io/${rname}/\nð¡ Para converter em APK: pwabuilder.com` : "")
      );
      setDone(true);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      <View style={[st.sheetRoot, { paddingTop: insets.top + 8, backgroundColor: CARD2 }]}>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
          <Text style={st.sectionLabel}>NOME DO REPOSITÃRIO</Text>
          <TextInput
          ...
          <Pressable onPress={() => setIsPrivate(v => !v)} style={st.toggleRow}>
            <View style={{ flex: 1 }}>
              <Text style={st.sectionLabel}>REPOSITÃRIO PRIVADO</Text>
              <Text style={[st.hint, { color: MUTED }]}>Apenas vocÃª poderÃ¡ ver</Text>
            </View>
          ...
          <Pressable onPress={() => setPages(v => !v)} style={st.toggleRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[st.hint, { color: MUTED }]}>Gera link HTTPS â vira PWA â APK via pwabuilder.com</Text>
            </View>
          ...
          <View style={st.infoBox}>
            <Text style={[st.hint, { color: "#60a5fa" }]}>ð¦ {files.length.toLocaleString()} arquivos â todos, sem limite</Text>
          </View>
          <MsgBox msg={msg} />
          {done && pushedOwner ? (
            <View style={{ gap: 8 }}>
              ...
              <Pressable onPress={() => Linking.openURL(`https://github.com/${pushedOwner}/${pushedRepo}/actions`)}
                ...
                <Text style={[st.actionBtnTxt, { color: ACCENT }]}>GitHub Actions â build APK</Text>
              </Pressable>
              {pages && (
                <>
                  ...
                  <Pressable onPress={() => Linking.openURL("https://www.pwabuilder.com/")}
                    ...
                    <Text style={[st.actionBtnTxt, { color: YELLOW }]}>Converter em APK â pwabuilder.com</Text>
                  </Pressable>
                </>
              )}
            </View>
...
export function APKBuilderModal({ visible, onClose }: Props) {
  ...
  useEffect(() => {
    AsyncStorage.multiGet(["apk_gh_token", "apk_cfg", "apk_neon", "apk_eas_token"]).then(pairs => {
      ...
      if (m["apk_gh_token"]) { setGhToken(m["apk_gh_token"]); setTokenInput(m["apk_gh_token"]); setGhUser("â salvo"); }
  ...
  async function verifyAndSaveToken(t: string) {
    ...
    try {
      ...
      Alert.alert("Token invÃ¡lido", String(e));
  ...
  function brainCopy() {
    ...
    const extSummary = Object.entries(exts)
      ...
      .join(", ");
    const context =
      `ð§  CONTEXTO DO PROJETO â ${cfg.appName || source}\n` +
      `ð¦ ${files.length.toLocaleString()} arquivos Â· ${source}\n` +
      `ð Pastas: ${[...topDirs].join(", ") || "raiz"}\n` +
      `ð Tipos: ${extSummary}\n` +
      `ð Package: ${cfg.appId || "nÃ£o definido"} Â· v${cfg.versionName}\n` +
      (neonDb ? `ðï¸ Neon DB: configurado\n` : "") +
      `\n[Cole isso no inÃ­cio da conversa com a IA para ela saber o projeto]`;
  ...
  async function handlePickZip() {
    try {
      ...
      setLoading(true); setLoadMsg(`ð¦ Lendo ${asset.name}...`);
      const base64 = await FileSystem.readAsStringAsync(asset.uri, { encoding: FileSystem.EncodingType.Base64 });
      setLoadMsg("ð Extraindo arquivos...");
      ...
      setLoadMsg(`â ${extracted.length.toLocaleString()} arquivos carregados â sem limite!`);
      setPgTab("editor"); setTreeOpen(true);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e) {
      setLoadMsg("â " + String(e));
    } finally { setLoading(false); }
  }

  async function handleGhImport() {
    const raw = ghRepo.trim();
    if (!raw) { Alert.alert("Digite o repositÃ³rio", "Formato: usuario/repo ou URL do GitHub"); return; }
    if (!ghToken) { Alert.alert("Token necessÃ¡rio", "Configure o token GitHub acima."); return; }
    let owner = "", repo = "";
    ...
    if (match) { owner = match[1]; repo = match[2].replace(/\.git$/, ""); }
      ...
      else { Alert.alert("Formato invÃ¡lido", "Use: usuario/repositorio"); return; }
    }
    setGhLoading(true); setGhMsg(`ð¥ Baixando ${owner}/${repo}...`);
    try {
      const repoFiles = await ghImportRepo(ghToken, owner, repo, "main", (p) => setGhMsg(`ð¥ ${p}`));
      const { name, id } = guessConfig(repoFiles, repo);
      saveCfg({ appName: name, appId: id });
      setFiles(repoFiles); setSource(`GitHub: ${owner}/${repo}`);
      setProjectReady(true); setResultB64("");
      setGhMsg(`â ${repoFiles.length.toLocaleString()} arquivos baixados â todos incluÃ­dos!`);
      setPgTab("editor"); setTreeOpen(true);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e) {
      setGhMsg("â " + String(e));
  ...
  async function handleBuild() {
    if (!files.length) { Alert.alert("Sem arquivos", "Importe um projeto primeiro."); return; }
    if (!cfg.appName.trim()) { Alert.alert("Nome obrigatÃ³rio", "Preencha o nome do app na aba Config."); return; }
    if (!cfg.appId.trim() || !cfg.appId.includes(".")) {
      Alert.alert("Package ID invÃ¡lido", "Use o formato: com.empresa.app"); return;
    }
    setBuilding(true); setBuildMsg("ð¨ Gerando projeto Android..."); setResultB64("");
    try {
      const b64 = await buildAndroidZip(cfg, files, source);
      setResultB64(b64);
      setBuildMsg(
        `â Projeto Android gerado! ${files.length.toLocaleString()} arquivos web incluÃ­dos.\n\n` +
        `ð PrÃ³ximos passos:\n` +
        `1. Baixe o ZIP ou envie ao GitHub\n` +
        `2. GitHub Actions compila o APK automaticamente (grÃ¡tis)\n` +
        `3. Baixe em: Actions â Ãºltimo build â Artifacts`
      );
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e) {
      setBuildMsg("â " + String(e));
  ...
  async function downloadResult() {
    ...
    try {
      if (Platform.OS === "web") {
        ...
        if (!dir) throw new Error("Cache directory nÃ£o disponÃ­vel");
  ...
  async function triggerEasBuild() {
    if (!easToken.trim()) {
      Alert.alert(
        "ð Token EAS necessÃ¡rio",
        "VÃ¡ para a aba Config e cole seu token EAS Build.\n\nexpo.dev â Account Settings â Access Tokens",
        [{ text: "Ir para Config", onPress: () => setPgTab("config") }, { text: "Cancelar", style: "cancel" }]
      );
      return;
    }
    setEasBuilding(true);
    setEasBuildMsg("ð Disparando build EAS...");
    try {
      const slug = (cfg.appName || "meuapp").replace(/[^a-zA-Z0-9]/g, "-").toLowerCase();
      ...
      if (json?.errors?.length) {
        setEasBuildMsg(`â Erro EAS: ${json.errors[0].message}`);
      } else if (json?.data?.build?.createBuild?.id) {
        const buildId = json.data.build.createBuild.id;
        setEasBuildMsg(`â Build disparado!\nID: ${buildId}\n\nAcompanhe em: expo.dev/accounts/me/projects/${slug}/builds`);
      } else {
        setEasBuildMsg("â ï¸ Resposta inesperada da API EAS. Verifique seu token e package ID.");
      }
    } catch (e) {
      setEasBuildMsg(`â Falha na conexÃ£o: ${String(e)}`);
    } finally {
  ...
  async function exportSourceZip() {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        const b64 = await zip.generateAsync({ type: "base64" });
        const dir = FileSystem.cacheDirectory;
        if (!dir) throw new Error("DiretÃ³rio nÃ£o disponÃ­vel");
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      <View style={[st.root, { backgroundColor: BG }]}>
        ...
        <View style={[st.header, { paddingTop: insets.top + 8 }]}>
          <View style={{ flex: 1 }}>
            <Text style={st.headerTitle}>ð¤ APK Builder</Text>
            <Text style={[st.hint, { color: MUTED }]} numberOfLines={1}>
              {projectReady ? `${files.length.toLocaleString()} arquivos Â· ${source}` : "Importe um projeto para comeÃ§ar"}
            </Text>
          </View>
          <View style={{ flexDirection: "row", gap: 6 }}>
            {projectReady && (
              <>
                ...
                <Pressable onPress={brainCopy} style={[st.iconBtn, { backgroundColor: brainCopied ? "#052e16" : "#1a0a2e" }]}>
                  <Text style={{ fontSize: 15 }}>{brainCopied ? "â" : "ð§ "}</Text>
                </Pressable>
              </>
            )}
        ...
        {pgTab === "import" && (
          <ScrollView style={{ flex: 1 }} contentContainerStyle={[st.content, { paddingBottom: 20 }]} keyboardShouldPersistTaps="handled">
            ...
            <SectionToggle label="ð TOKEN GITHUB" open={secToken} onToggle={() => setSecToken(v => !v)}>
              {ghUser ? (
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <View style={st.badge}><Text style={st.badgeTxt}>â {ghUser}</Text></View>
              ...
              <Text style={[st.hint, { color: MUTED, marginTop: 6 }]}>
                GitHub â Settings â Developer settings â Personal access tokens â repo, workflow
              </Text>
            </SectionToggle>

            <SectionToggle label="ð¦ IMPORTAR ZIP / TAR.GZ" open={secImport} onToggle={() => setSecImport(v => !v)}>
              <Pressable
              ...
              >
                {loading ? <ActivityIndicator color={ACCENT} /> : (
                  <>
                    <Text style={{ fontSize: 28 }}>{projectReady ? "â" : "ð¦"}</Text>
                    ...
                    <Text style={[st.hint, { color: MUTED }]}>
                      {projectReady ? source : "ZIP, TAR.GZ, TGZ â qualquer projeto web"}
                    </Text>
                  </>
                )}
              </Pressable>
            ...
            <SectionToggle label="ð CLONAR REPOSITÃRIO GITHUB" open={secGh} onToggle={() => setSecGh(v => !v)}>
              <TextInput
              ...
              >
                ...
                <Text style={st.btnTxt}>{ghLoading ? "Baixando..." : "Clonar repositÃ³rio"}</Text>
              </Pressable>
              <MsgBox msg={ghMsg} />
            </SectionToggle>
            ...
            {projectReady && (
              <View style={st.card}>
                <Text style={[st.sectionLabel, { marginBottom: 10 }]}>AÃÃES RÃPIDAS</Text>
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={[st.content, { paddingBottom: 20 }]} keyboardShouldPersistTaps="handled">
            <View style={st.card}>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={st.fieldLabel}>VERSÃO</Text>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[st.fieldLabel, { marginBottom: 6 }]}>ORIENTAÃÃO</Text>
            ...
            <View style={st.card}>
              <Text style={[st.sectionLabel, { marginBottom: 10 }]}>ðï¸ BANCO NEON (POSTGRESQL)</Text>
              ...
              {neonDb.trim() ? (
                <View style={[st.infoBox, { marginTop: 8, borderColor: GREEN + "44", backgroundColor: "#052e16" }]}>
                  <Text style={[st.hint, { color: GREEN }]}>â Neon DB configurado e salvo localmente</Text>
                </View>
              ) : null}
            </View>

            <View style={st.card}>
              <Text style={[st.sectionLabel, { marginBottom: 10 }]}>ð TOKEN EAS BUILD (EXPO)</Text>
              <Text style={[st.hint, { color: MUTED, marginBottom: 8 }]}>
                Token EAS para compilar APK via Expo â expo.dev â Access Tokens
              </Text>
              <TextInput
              ...
              {easToken.trim() ? (
                <View style={[st.infoBox, { marginTop: 8, borderColor: ACCENT + "44", backgroundColor: ACCENT + "11" }]}>
                  <Text style={[st.hint, { color: ACCENT }]}>â EAS token salvo localmente</Text>
                </View>
              ) : null}
            </View>
          </ScrollView>
        ...
        {pgTab === "build" && (
          <ScrollView style={{ flex: 1 }} contentContainerStyle={[st.content, { paddingBottom: 20 }]}>
            <View style={st.card}>
              ...
              {([
                ["Nome", cfg.appName || "(nÃ£o definido)"],
                ["Package", cfg.appId || "(nÃ£o definido)"],
                ["VersÃ£o", `${cfg.versionName} (build ${cfg.versionCode})`],
                ["Min SDK", `API ${cfg.minSdk}+`],
                ["OrientaÃ§Ã£o", cfg.orientation],
            ...
            <View style={st.card}>
              ...
              <Text style={[st.hint, { color: MUTED, marginBottom: 12, lineHeight: 18 }]}>
                Gera projeto Android Studio com WebView nativo + GitHub Actions. FaÃ§a push â APK gerado automaticamente (grÃ¡tis).
              </Text>
              <Pressable
                onPress={handleBuild} disabled={building || !files.length}
                style={[st.bigBtn, (building || !files.length) && { opacity: 0.5 }]}
              >
                {building ? <ActivityIndicator color="#fff" /> : <Text style={{ fontSize: 20 }}>ð¤</Text>}
                <Text style={st.bigBtnTxt}>{building ? "Gerando..." : "Gerar Projeto Android"}</Text>
              </Pressable>
              <SectionToggle label="ð LOG DO BUILD" open={secBuildLog} onToggle={() => setSecBuildLog(v => !v)}>
                <MsgBox msg={buildMsg} />
              </SectionToggle>
              {resultB64 ? (
            ...
            <View style={st.card}>
              <Text style={[st.sectionLabel, { marginBottom: 4 }]}>ð EAS BUILD (EXPO)</Text>
              {/* Status do token */}
              <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 12 }}>
                ...
                <Text style={[st.hint, { color: easToken.trim() ? "#22c55e" : "#f59e0b" }]}>
                  {easToken.trim() ? "Token EAS configurado â pronto para disparar" : "Token EAS nÃ£o configurado â vÃ¡ em Config"}
                </Text>
                {!easToken.trim() && (
                  <Pressable onPress={() => setPgTab("config")} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
                    <Text style={{ color: "#7c3aed", fontSize: 11, fontWeight: "700" }}>â Config</Text>
                  </Pressable>
                )}
              </View>
              {/* BotÃ£o DISPARAR */}
              ...
              >
                {easBuilding
                  ...
                  : <Text style={{ fontSize: 18 }}>ð</Text>}
                <Text style={st.bigBtnTxt}>{easBuilding ? "Disparando..." : "DISPARAR BUILD APK"}</Text>
              </Pressable>
              {easBuildMsg ? (
                <View style={[st.infoBox, {
                  marginBottom: 8,
                  borderColor: easBuildMsg.startsWith("â") ? "#22c55e44" : easBuildMsg.startsWith("â") ? "#ef444444" : "#7c3aed44",
                  backgroundColor: easBuildMsg.startsWith("â") ? "#052e16" : easBuildMsg.startsWith("â") ? "#1a0000" : "#1a0a2e",
                }]}>
                  <Text style={[st.hint, {
                    color: easBuildMsg.startsWith("â") ? "#22c55e" : easBuildMsg.startsWith("â") ? "#f87171" : "#c4b5fd",
                    lineHeight: 18,
                  }]}>{easBuildMsg}</Text>
                </View>
              ) : null}
            ...
            <View style={st.card}>
              <Text style={[st.sectionLabel, { marginBottom: 8 }]}>PWA â APK VIA GITHUB PAGES</Text>
              ...
              {([
                ...
                ["4", "Bubblewrap (Google)", "TWA nativo â chrome WebView otimizado"],
              ] as const).map(([n, t, d]) => (
        ...
        {/* ââ ÃRVORE DE ARQUIVOS (painel inferior persistente) ââ */}
        <View style={[st.treePanel, { paddingBottom: Math.max(insets.bottom, 8) }]}>
          <Pressable onPress={() => setTreeOpen(v => !v)} style={st.treePanelBar}>
            ...
            <Text style={[st.sectionLabel, { color: treeOpen ? ACCENT : MUTED }]}>
              {files.length > 0 ? `ÃRVORE â ${files.length.toLocaleString()} ARQUIVOS` : "ÃRVORE DE ARQUIVOS"}
            </Text>
            {files.length > 0 && (
            ...
            {projectReady && (
              <Pressable onPress={brainCopy} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                ...
                <Text style={{ fontSize: 16 }}>{brainCopied ? "â" : "ð§ "}</Text>
              </Pressable>
            )}
