...
const VOICE_MODES: { key: string; label: string; icon: string; color: string; prompt: string; showCode?: boolean }[] = [
  {
    key: "copiloto",
    label: "Copiloto",
    icon: "cpu",
    color: "#10b981",
    showCode: true,
    prompt: `VocÃª Ã© um COPILOTO de desenvolvimento â age como um par programador experiente que conversa de forma natural e tambÃ©m entrega cÃ³digo quando necessÃ¡rio.

COMPORTAMENTO:
- Fale naturalmente em portuguÃªs do Brasil como uma pessoa real, sem asteriscos ou markdown na parte falada
- Quando precisar mostrar cÃ³digo, comando de terminal ou arquivo, coloque em bloco de cÃ³digo com a linguagem correta (ex: \`\`\`bash, \`\`\`javascript, \`\`\`python)
- Para comandos de terminal, use SEMPRE \`\`\`bash ou \`\`\`sh para que o usuÃ¡rio possa executar com um toque
- Para cÃ³digo de arquivos, use a linguagem correta para que o usuÃ¡rio possa aplicar com um toque
- Seja proativo: apÃ³s explicar o que fazer, jÃ¡ mostre o cÃ³digo/comando pronto
- Para tarefas grandes, liste os passos e execute um por vez
- ApÃ³s cada aÃ§Ã£o, explique o que foi feito e o prÃ³ximo passo sugerido`,
  },
  {
    key: "dev",
    label: "Dev",
...
export default function VoiceAssistant({ visible, onClose }: Props) {
  ...
  const { getActiveAIProvider, settings, activeProject, activeFile, aiMemory } = useApp();
  ...
  const buildProjectContext = useCallback(() => {
    if (!voiceMode.showCode) return "";
    let ctx = "";
    if (activeProject) {
      ctx += `\n\nð PROJETO ABERTO: ${activeProject.name} (${activeProject.files.length} arquivo${activeProject.files.length !== 1 ? "s" : ""})`;
      if (activeFile) {
        ctx += `\nð ARQUIVO ATIVO: ${activeFile.name} (${activeFile.language})\n\`\`\`${activeFile.language}\n${activeFile.content}\n\`\`\``;
      }
      const outros = activeProject.files.filter(f => f.id !== activeFile?.id);
      if (outros.length > 0) {
        ctx += `\n\nDEMAIS ARQUIVOS:`;
    ...
    if (aiMemory.length > 0) {
      ctx += `\n\nð§  MEMÃRIA: ${aiMemory.slice(-20).map(e => `[${e.category}] ${e.content}`).join(" | ")}`;
    }
    return ctx;
  }, [voiceMode.showCode, activeProject, activeFile, aiMemory]);
  ...
  const handleSpeechResult = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const fullPrompt = voiceMode.prompt + buildProjectContext();
        await callAIVoice(provider, history, (chunk) => {
          reply += chunk;
          setCurrentReply(reply);
        }, apiBase, fullPrompt, settings.geminiDirectKey);
    ...
    [getActiveAIProvider, apiBase, speak, voiceMode, buildProjectContext]
  );
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={handleClose}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        >
          {messages.map((m) => (
            ...
            >
              ...
              {m.role === "assistant" && voiceMode.showCode ? (
                <MessageRenderer content={m.content} isUser={false} showApply />
              ) : (
          ...
          {currentReply ? (
            <View style={[styles.bubble, styles.bubbleAssistant, { backgroundColor: colors.card, borderColor: stateColor }]}>
              <Text style={[styles.bubbleLabel, { color: stateColor }]}>IA (digitando...)</Text>
              {voiceMode.showCode ? (
                <MessageRenderer content={currentReply} isUser={false} showApply={false} />
              ) : (
        ...
        {/* Chips de aÃ§Ã£o rÃ¡pida â sÃ³ no modo Copiloto */}
        {voiceMode.showCode && messages.length === 0 && voiceState === "idle" && (
          <View style={{ paddingHorizontal: 12, paddingBottom: 8 }}>
            <Text style={{ color: colors.mutedForeground, fontSize: 11, marginBottom: 6, textAlign: "center" }}>
              Diga ou toque para comeÃ§ar:
            </Text>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, justifyContent: "center" }}>
              {[
                "Explique o arquivo atual",
                "Refatore este cÃ³digo",
                "Crie um componente",
                ...
                <TouchableOpacity
                  key={chip}
                  onPress={() => handleSpeechResult(chip)}
                  style={{
                    paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16,
                    backgroundColor: "#10b98115", borderWidth: 1, borderColor: "#10b98133",
                  }}
                >
                  <Text style={{ color: "#10b981", fontSize: 11, fontWeight: "600" }}>{chip}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
