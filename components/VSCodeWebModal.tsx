...
export default function VSCodeWebModal({ visible, onClose }: Props) {
  ...
  const handlePushAndOpen = useCallback(async () => {
    ...
    setStep("pushing");
    setLogs(["ð Iniciando envio para o GitHubâ¦"]);
    setError("");

    try {
      const user = await getUser(token);
      const owner = user.login;
      addLog(`ð¤ Conta: ${owner}`);

      // Criar repositÃ³rio
      addLog(`ð Criando repositÃ³rio "${repo}"â¦`);
      try {
        await createRepo(token, repo, `${appName} â DevMobile`, false);
        addLog("â RepositÃ³rio criado.");
      } catch (e: any) {
        if (e.message?.includes("422") || e.message?.includes("already exists") || e.message?.includes("name already exists")) {
          addLog("â¹ï¸ RepositÃ³rio jÃ¡ existe â usando existente.");
        } else throw e;
      }
      ...
      addLog(`ð¤ Enviando ${fileList.length} arquivo(s)â¦`);
      await pushFiles(token, owner, repo, fileList, `${appName} â enviado pelo DevMobile`);
      addLog("â Projeto enviado!");

      const url = `https://vscode.dev/github/${owner}/${repo}`;
      addLog(`ð» Abrindo: ${url}`);
      setVsCodeUrl(url);
      setStep("done");
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: insets.bottom + 40 }}>
          ...
          <View style={[s.infoBox, { backgroundColor: "#007acc11", borderColor: "#007acc33" }]}>
            ...
            <Text style={[s.infoText, { color: colors.mutedForeground }]}>
              Seu projeto vai ser enviado para o GitHub e aberto no VS Code Web â com todos os arquivos, extensÃµes, autocomplete e terminal integrado. Ã o VS Code real, no navegador.
            </Text>
          </View>

          {!hasToken ? (
            /* Sem token â abre vscode.dev vazio */
            <View style={{ gap: 12 }}>
              <View style={[s.infoBox, { backgroundColor: "#f59e0b11", borderColor: "#f59e0b33" }]}>
                <Text style={[s.infoTitle, { color: "#f59e0b" }]}>â ï¸ GitHub nÃ£o configurado</Text>
                <Text style={[s.infoText, { color: colors.mutedForeground }]}>
                  Sem o GitHub, o VS Code Web abre sem seus arquivos. Configure em Menu â GitHub primeiro para abrir com o projeto completo.
                </Text>
              </View>
              <TouchableOpacity
            ...
            <View style={{ gap: 12 }}>
              ...
              <Text style={{ fontSize: 11, color: colors.mutedForeground }}>
                O projeto serÃ¡ enviado para github.com/{ghConfig ? "..." : "seu-usuario"}/{repoName || defaultRepo}
              </Text>
              ...
              <View style={[s.infoBox, { backgroundColor: colors.card, borderColor: colors.border, gap: 8 }]}>
                ...
                {[
                  "Cria ou usa repositÃ³rio GitHub",
              ...
              >
                <Text style={{ color: colors.mutedForeground, fontSize: 12 }}>Abrir vscode.dev sem enviar â</Text>
              </TouchableOpacity>
            </View>
          ) : step === "pushing" ? (
            <View style={{ gap: 10 }}>
              <View style={[s.logBox, { backgroundColor: "#000", borderColor: colors.border }]}>
                <ScrollView>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 4 }}>
                    ...
                    <Text style={{ color: "#007acc88", fontSize: 11 }}>Enviandoâ¦</Text>
                  </View>
                </ScrollView>
              </View>
            </View>
            ...
            <View style={{ gap: 12 }}>
              <View style={[s.infoBox, { backgroundColor: "#22c55e11", borderColor: "#22c55e33" }]}>
                <Text style={[s.infoTitle, { color: "#4ade80" }]}>â Projeto enviado e aberto!</Text>
                <Text style={[s.infoText, { color: colors.mutedForeground }]}>
                  O VS Code Web foi aberto com seu projeto completo. Se nÃ£o abriu, toque no botÃ£o abaixo.
                </Text>
              </View>
              ...
              <TouchableOpacity onPress={handleReset} style={{ alignItems: "center", paddingVertical: 8 }}>
                <Text style={{ color: colors.mutedForeground, fontSize: 12 }}>â© Usar outro repositÃ³rio</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <View style={{ gap: 12 }}>
              <View style={[s.infoBox, { backgroundColor: "#ef444411", borderColor: "#ef444433" }]}>
                <Text style={[s.infoTitle, { color: "#f87171" }]}>â Erro ao enviar</Text>
