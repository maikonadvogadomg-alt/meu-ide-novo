...
export default function HtmlPlayground({ visible, onClose }: Props) {
  ...
  const exportCode = async () => {
    ...
    try {
      if (Platform.OS === "web") {
        ...
        if (canShare) {
          await Sharing.shareAsync(htmlFile.uri, { mimeType: "text/html", dialogTitle: "Exportar cÃ³digo" });
        } else {
  ...
  const LAYOUTS: { key: Layout; icon: string; tip: string }[] = [
    ...
    { key: "preview", icon: "maximize-2", tip: "SÃ³ preview" },
  ];
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: "row", backgroundColor: "#161b22", paddingHorizontal: 10, paddingVertical: 3, gap: 12 }}>
        <Text style={{ color: "#484f58", fontSize: 10, fontFamily: Platform.OS === "ios" ? "Menlo" : "monospace" }}>
          {lineCount} linhas Â· {code.length} chars Â· {mode.toUpperCase()}
        </Text>
        {!autoRender && (
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: "#0d1117" }}>
        ...
        <View style={[s.header, { paddingTop: insets.top + 4 }]}>
          ...
          <Text style={s.headerTitle}>ð® Playground</Text>
        ...
        {/* ââ Save name input inline ââ */}
        {showSaveInput && (
        ...
        {layout === "stacked" ? (
          ...
          <View style={{ flex: 1 }}>
            {PreviewPanel}
            {/* Mini botÃ£o para voltar ao editor */}
            <TouchableOpacity
        ...
        <View style={s.savesOverlay}>
          <View style={[s.savesPanel, { backgroundColor: "#161b22", borderColor: "#30363d" }]}>
            ...
            <View style={[s.savesHeader, { borderBottomColor: "#30363d" }]}>
              <Text style={[s.savesTitle, { color: "#e6edf3" }]}>ð¾ Meus CÃ³digos Salvos</Text>
              ...
              {filteredSaves.length === 0 ? (
                <View style={{ alignItems: "center", paddingTop: 40, gap: 10 }}>
                  ...
                  <Text style={{ color: "#8b949e", textAlign: "center" }}>
                    {saves.length === 0
                      ? "Nenhum cÃ³digo salvo ainda.\nUse o botÃ£o Salvar para guardar."
