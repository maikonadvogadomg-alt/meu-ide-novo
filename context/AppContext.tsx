...
export interface AppSettings {
  ...
  /** Servidor habilitado (Termux/VPS). Desligado por padrÃ£o â app funciona sem servidor. */
...
// Chave legada do AsyncStorage (para migraÃ§Ã£o de dados antigos)
...
// Carrega os arquivos de um projeto do sistema de arquivos (com migraÃ§Ã£o automÃ¡tica do AsyncStorage)
async function readProjectFiles(projectId: string): Promise<ProjectFile[]> {
  // Tenta o sistema de arquivos primeiro (nova versÃ£o, sem limite)
  try {
  ...
  // MigraÃ§Ã£o: lÃª do AsyncStorage (versÃ£o antiga) e move para FileSystem
  try {
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  async function loadData() {
    ...
    if (p && Array.isArray(p)) {
      ...
      // Migra automaticamente do AsyncStorage se necessÃ¡rio
      ...
      // Restaura projeto e arquivo ativos da Ãºltima sessÃ£o
      if (savedProjectId) {
  ...
  // Salva metadados dos projetos (SEM arquivos â arquivos vÃ£o para FileSystem via persistProjectFiles)
  ...
  async function save(key: string, data: unknown) {
    try {
      if (key === STORAGE_KEYS.PROJECTS) {
        ...
        // Salva APENAS metadados â arquivos sÃ£o salvos separadamente via saveProjectFiles
  ...
  const setActiveProject = useCallback((project: Project | null) => {
    ...
    // Define o projeto imediatamente para que a UI responda rÃ¡pido
    ...
    // (migraÃ§Ã£o de sessÃ£o antiga ou race condition na inicializaÃ§Ã£o)
  ...
  const updateFile = useCallback(
    (projectId: string, fileId: string, content: string) => {
      setProjects((prev) => {
        ...
        // Salva sÃ³ os arquivos do projeto afetado â rÃ¡pido mesmo com 5.000 arquivos
        const proj = next.find((p) => p.id === projectId);
        if (proj) saveProjectFiles(projectId, proj.files);
        return next;
      });
  ...
  const importGitRepo = useCallback(
    async (url: string, token: string, provider: "github" | "gitlab", onProgress?: (cur: number, total: number, phase: string) => void): Promise<Project> => {
      ...
      if (provider === "github") {
        // Usa zipball â 1 requisiÃ§Ã£o, suporta 38.000+ arquivos sem rate limit
        ...
        if (match && token) {
          ...
          // Busca todos os arquivos com paginaÃ§Ã£o (sem limite)
      ...
      if (files.length === 0) {
        throw new Error("Nenhum arquivo foi importado. Verifique se o repositÃ³rio Ã© pÃºblico ou se o token tem permissÃ£o de acesso.");
      }
      ...
      // Coloca o projeto no estado IMEDIATAMENTE â UI aparece na hora
      ...
      // Salva no disco em segundo plano â nÃ£o bloqueia a UI
  ...
  const pushToGit = useCallback(
    async (
      ...
      if (!isGitLab) {
        // ââ GitHub: Git Trees API âââââââââââââââââââââââââââââââââââââââââââââ
        // Abordagem em LOTE: 1 commit para todos os arquivos, nÃ£o 1 PUT por arquivo
        const match = clean.match(/github\.com\/([^/]+)\/([^/]+)/);
        if (!match) throw new Error("URL do GitHub invÃ¡lida. Formato: https://github.com/usuario/repositorio");
        const [, owner, repo] = match;
        const api = `https://api.github.com/repos/${owner}/${repo}`;

        // Passo 1: obter SHA do Ãºltimo commit do branch
        onProgress?.(0, total, "Conectando ao repositÃ³rio...");
        const refRes = await fetch(`${api}/git/ref/heads/${branch}`, { headers: ghHeaders() });
        if (!refRes.ok) {
          // Branch nÃ£o existe â tentar criar a partir do commit raiz
          if (refRes.status === 404) throw new Error(`Branch "${branch}" nÃ£o existe. Crie-o no GitHub primeiro ou use "main".`);
          const e = await refRes.json().catch(() => ({}));
          throw new Error((e as any)?.message || `Erro ao acessar o repositÃ³rio (${refRes.status})`);
        }
        ...
        if (treeNodes.length === 0) throw new Error("Nenhum arquivo pÃ´de ser enviado.");

        // Passo 3: criar uma Ãºnica Ã¡rvore com todos os arquivos
        ...
        if (!treeRes.ok) {
          ...
          throw new Error((e as any)?.message || "Falha ao criar Ã¡rvore de arquivos");
        }
        ...
        const commitRes = await fetch(`${api}/git/commits`, {
          ...
          headers: ghHeaders(),
          body: JSON.stringify({
            message: `DevMobile: ${treeNodes.length} arquivo(s) â ${now}`,
            tree: treeData.sha,
        ...
        // ââ GitLab: Commits API (batch) ââââââââââââââââââââââââââââââââââââââââ
        const match = clean.match(/gitlab\.com\/([^?#]+)/);
        if (!match) throw new Error("URL do GitLab invÃ¡lida. Formato: https://gitlab.com/usuario/repositorio");
        const projectPath = encodeURIComponent(match[1]);
        ...
        const commitRes = await fetch(`${api}/repository/commits`, {
          ...
          body: JSON.stringify({
            branch,
            commit_message: `DevMobile: ${actions.length} arquivo(s) â ${now}`,
            actions,
          }),
        });
        if (!commitRes.ok) {
