...
export async function ghGetUser(token: string): Promise<GhUser> {
  ...
  if (!r.ok) throw new Error(`Token invÃ¡lido: ${r.status}`);
  return r.json();
}
...
async function fetchRepoInfo(
  ...
  if (!r.ok) throw new Error(`Repo nÃ£o encontrado: ${r.status}`);
  return r.json();
}
...
async function _extractZip(
  ...
  onProgress(`â ${files.length} arquivos importados`);
  return files;
}
...
export async function ghImportRepo(
  ...
  onProgress("Verificando repositÃ³rio...");
  ...
  if (!r.ok) throw new Error(`Erro ao baixar repositÃ³rio: ${r.status}`);
