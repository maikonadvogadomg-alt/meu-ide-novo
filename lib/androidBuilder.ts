...
export async function buildAndroidZip(
  ...
  zip.file(
    "README.md",
    `# ${cfg.appName} â Projeto Android (Pure WebView)\n` +
      `## Origem: ${source}\n` +
      (fromSubfolder ? `## Arquivos da pasta: ${fromSubfolder}/\n` : "") +
      `## Como compilar\n` +
      `### OpÃ§Ã£o 1 â GitHub Actions (automÃ¡tico, gratuito)\n` +
      `1. FaÃ§a push deste projeto para o GitHub\n` +
      `2. O workflow .github/workflows/build-apk.yml roda automaticamente\n` +
      `3. Baixe o APK em: RepositÃ³rio â Actions â Ãºltimo build â Artifacts\n\n` +
      `### OpÃ§Ã£o 2 â Android Studio\n` +
      `1. Extraia este ZIP\n` +
      `2. Android Studio â File â Open â pasta android/\n` +
      `3. Build â Build APK(s)\n` +
      `4. APK: android/app/build/outputs/apk/debug/app-debug.apk\n\n` +
      `## Config\n` +
      `- Package: ${cfg.appId}\n` +
      `- VersÃ£o: ${cfg.versionName} (${cfg.versionCode})\n` +
      `- Android mÃ­nimo: ${cfg.minSdk}+\n` +
      `- Arquivos web: ${files.length}\n` +
      (hasIndexHtml
        ? ""
        : "\nâ ï¸ AVISO: index.html nÃ£o encontrado nos arquivos importados!\n")
  );
...
public class MainActivity extends AppCompatActivity {
  ...
  ghWorkflows.file(
    "build-apk.yml",
    `name: ð¤ Build APK â ${cfg.appName}
