...
export async function extractArchive(
  ...
  if (bytes.length === 0) throw new Error("Arquivo invÃ¡lido ou vazio");
  ...
  if (name.endsWith(".tar.gz") || name.endsWith(".tgz")) {
    ...
    try {
      ...
      try {
        ...
        throw new Error("NÃ£o foi possÃ­vel descompactar o arquivo: " + String(e));
      }
    }
  ...
  try {
    ...
    throw new Error("Arquivo ZIP invÃ¡lido: " + String(e));
  }
...
export function decodeFileToText(f: ProjectFile): string {
  ...
  if (f.isBinary || isBinaryPath(f.path)) {
    const kb = f.size ? ` Â· ${(f.size / 1024).toFixed(1)} KB` : "";
    return `[Arquivo binÃ¡rio${kb} â nÃ£o pode ser visualizado como texto]`;
  }
  try {
    const clean = cleanBase64(f.data);
    let raw: string;
    try {
      ...
      return "(erro ao decodificar: base64 invÃ¡lido)";
    }
    let nonPrint = 0;
    ...
    if (checkLen > 0 && nonPrint / checkLen > 0.1) {
      const kb = f.size ? ` Â· ${(f.size / 1024).toFixed(1)} KB` : "";
      return `[Arquivo binÃ¡rio${kb} â conteÃºdo nÃ£o textual]`;
    }
    const bytes = new Uint8Array(raw.length);
    for (let i = 0; i < raw.length; i++) bytes[i] = raw.charCodeAt(i);
    return new TextDecoder("utf-8", { fatal: false }).decode(bytes);
