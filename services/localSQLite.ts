...
export async function formatSQLResult(
  ...
  try {
    ...
    if (result.isSelect) {
      ...
      const header = cols.map((c, i) => c.padEnd(widths[i])).join(" â ");
      const sep = widths.map((w) => "â".repeat(w)).join("ââ¼â");
      const rowLines = result.rows.map((r) =>
        cols.map((c, i) => String(r[c] ?? "NULL").padEnd(widths[i])).join(" â ")
      );
      return [header, sep, ...rowLines, `\n(${result.rows.length} linha${result.rows.length !== 1 ? "s" : ""})`].join("\n");
    } else {
      return `â OK â ${result.changes ?? 0} linha(s) afetada(s)${result.lastInsertRowId ? ` Â· ROWID=${result.lastInsertRowId}` : ""}`;
    }
  } catch (e: unknown) {
