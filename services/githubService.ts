...
export async function getUser(token: string): Promise<GHUser> {
  ...
  if (!res.ok) throw new Error(`Token invÃ¡lido (${res.status})`);
  return res.json();
}
...
export async function cloneRepo(
  ...
  onProgress?.(0, 100, "Conectando ao repositÃ³rio...");

  const repoRes = await ghFetch(`/repos/${owner}/${repo}`, token);
  if (!repoRes.ok) throw new Error(`RepositÃ³rio nÃ£o encontrado (${repoRes.status})`);
  const repoData: GHRepo = await repoRes.json();
  const defaultBranch = branch || repoData.default_branch || "main";

  // Download ZIP usando fetch + arrayBuffer â funciona em todas as plataformas
  // ArrayBuffer nÃ£o Ã© limitado pelo truncamento de 16 MB do Hermes (que afeta sÃ³ strings)
  ...
  try {
    ...
    if (!zipRes.ok) {
      throw new Error(`Falha ao baixar repositÃ³rio (${zipRes.status}). Verifique o token e tente novamente.`);
    }
    onProgress?.(20, 100, "Recebendo dados...");
    zipData = await zipRes.arrayBuffer();
  } catch (e: any) {
    if (e?.message?.includes("status")) throw e;
    throw new Error(`Erro de rede ao baixar repositÃ³rio: ${e?.message || String(e)}`);
  }
...
export async function clonePublicUrl(
  ...
  if (!match) throw new Error("URL invÃ¡lida. Use: https://github.com/usuario/repositorio");
...
export async function createRepo(
  ...
  if (!res.ok) {
    ...
    throw new Error((err as any)?.message || `Falha ao criar repositÃ³rio (${res.status})`);
  }
  return res.json();
}
...
async function getDefaultBranch(token: string, owner: string, repo: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`RepositÃ³rio "${owner}/${repo}" nÃ£o encontrado.`);
  const data: GHRepo = await res.json();
...
async function getLatestCommitSha(
  ...
  if (!res.ok) throw new Error(`Branch "${branch}" nÃ£o encontrada.`);
...
export async function pushFiles(
  ...
  if (!treeRes.ok) throw new Error("Falha ao criar Ã¡rvore de arquivos");
  ...
  if (!refRes.ok) throw new Error("Falha ao atualizar referÃªncia do branch");
...
export async function makeRepoPublic(
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error((err as any)?.message || `NÃ£o foi possÃ­vel tornar o repositÃ³rio pÃºblico (${res.status})`);
  }
}
