...
export default function App() {
  ...
  const saveUrl = async (url: string) => {
    ...
    if (!clean.startsWith("http")) {
      Alert.alert("URL invÃ¡lida", "Cole a URL completa comeÃ§ando com https://");
      return;
    }
  ...
  const clearUrl = async () => {
    Alert.alert("Redefinir servidor", "Isso vai te levar para a tela de configuraÃ§Ã£o. Continuar?",
  ...
  if (screen === "loading") return (
    <View style={s.splash}>
      ...
      <Text style={s.splashIcon}>âï¸</Text>
      <Text style={s.splashTitle}>Assistente JurÃ­dico IA</Text>
  ...
  if (screen === "setup") return (
    <KeyboardAvoidingView style={s.setup} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      <ScrollView contentContainerStyle={s.setupContent}>
        <Text style={s.setupIcon}>âï¸</Text>
        <Text style={s.setupTitle}>Assistente JurÃ­dico IA</Text>
        <Text style={s.setupSub}>Maikon Caldeira â OAB/MG 183712</Text>
        <View style={s.card}>
          <Text style={s.cardTitle}>ð URL do Servidor</Text>
          <Text style={s.cardDesc}>Cole o endereÃ§o onde o Assistente JurÃ­dico estÃ¡ hospedado (Railway, VPS, etc).</Text>
          <TextInput
          ...
          <TouchableOpacity style={[s.btn, !inputUrl.trim() && s.btnOff]} disabled={!inputUrl.trim()} onPress={() => saveUrl(inputUrl)}>
            <Text style={s.btnText}>Conectar â</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    ...
    <View style={s.container}>
      ...
      {showSettings && (
        <View style={s.overlay}>
          <View style={s.settingsCard}>
            <Text style={s.settingsTitle}>âï¸ ConfiguraÃ§Ãµes</Text>
      ...
      {webError && !showSettings && (
        <View style={s.errorScreen}>
          <Text style={s.errorIcon}>ð¡</Text>
          <Text style={s.errorTitle}>Servidor nÃ£o encontrado</Text>
          <Text style={s.errorDesc}>NÃ£o foi possÃ­vel conectar a:{"\n"}{serverUrl}</Text>
...
const COMO_COMPILAR = `# Assistente JurÃ­dico IA â Como Compilar o APK

## O que Ã© este projeto
App Android nativo que abre o Assistente JurÃ­dico IA num WebView.
- Funciona como app real instalado no celular
- BotÃ£o voltar do Android funciona normalmente
...
// ââ Templates dos modelos top de linha ââââââââââââââââââââââââââââââââââââââ
...
// Groq Ã© GRATUITO â crie sua chave em console.groq.com
app.post('/api/chat', async (req, res) => {
  const { messages, system } = req.body;
  if (!GROQ_KEY) return res.status(400).json({ erro: 'GROQ_API_KEY nÃ£o configurada no .env' });
...
app.listen(PORT, () => {
  ...
  console.log(\`Gratuito: sim â console.groq.com\`);
});
`;
...
# Modelos disponÃ­veis (todos gratuitos):
# llama-3.3-70b-versatile  â recomendado (mais inteligente)
# llama-3.1-8b-instant     â mais rÃ¡pido
# mixtral-8x7b-32768       â contexto enorme
# gemma2-9b-it             â Google Gemma
...
## O que Ã© o Groq?
Groq Ã© uma plataforma de IA **100% gratuita** que oferece os melhores modelos de linguagem abertos
com velocidade extremamente rÃ¡pida. Inclui Llama 3.3 70B, Mixtral, Gemma e outros.
...
|--------|-----------|-------------|
| llama-3.3-70b-versatile | 6.000 | Tudo â mais inteligente |
| llama-3.1-8b-instant | 30.000 | Velocidade mÃ¡xima |
| mixtral-8x7b-32768 | 5.000 | Contexto longo (32k) |
| gemma2-9b-it | 15.000 | AnÃ¡lise de texto |
`;
...
// Claude da Anthropic â modelos mais avanÃ§ados do mundo
app.post('/api/chat', async (req, res) => {
  const { messages, system } = req.body;
  if (!CLAUDE_KEY) return res.status(400).json({ erro: 'ANTHROPIC_API_KEY nÃ£o configurada no .env' });
...
# claude-haiku-4-20250514    â mais rÃ¡pido e barato
# claude-sonnet-4-20250514   â melhor custo-benefÃ­cio
# claude-opus-4-20250514     â mais inteligente do mundo
...
// Google Gemini â gratuito com limite generoso
app.post('/api/chat', async (req, res) => {
  const { messages, system } = req.body;
  if (!GEMINI_KEY) return res.status(400).json({ erro: 'GEMINI_API_KEY nÃ£o configurada no .env' });
...
# gemini-2.0-flash           â recomendado (rÃ¡pido + inteligente)
# gemini-2.5-pro             â mais avanÃ§ado
# gemini-2.5-flash           â ultra-rÃ¡pido
...
// ConfiguraÃ§Ã£o de todos os provedores
const PROVIDERS = {
  groq: {
    nome: 'Groq (Gratuito)',
...
// Rota principal: escolhe provedor automaticamente ou por parÃ¢metro
app.post('/api/chat', async (req, res) => {
...
app.listen(PORT, () => {
  ...
  console.log(\`Provedores ativos: \${configurados.join(', ') || 'nenhum â configure .env'}\`);
});
`;

const MULTI_IA_ENV = `# âââââââââââââââââââââââââââââââââââââââââââ
# API Multi-Modelo â configure pelo menos UM
# âââââââââââââââââââââââââââââââââââââââââââ

# GROQ â GRATUITO, mais rÃ¡pido
# Crie em: https://console.groq.com/keys
GROQ_API_KEY=gsk_SUA_CHAVE_AQUI

# GEMINI â GRATUITO, Google
# Crie em: https://aistudio.google.com/app/apikey
GEMINI_API_KEY=AIzaSUA_CHAVE_AQUI

# CLAUDE â Pago, mais inteligente do mundo
# Crie em: https://console.anthropic.com/keys
ANTHROPIC_API_KEY=sk-ant-SUA_CHAVE_AQUI

# OPENAI â Pago, GPT-4o
...
const PKG_IA = `{
  "name": "api-ia",
  "version": "1.0.0",
  "description": "API com integraÃ§Ã£o de IA",
  "main": "server.js",
  "scripts": {
...
export const FEATURED_PROJECTS: FeaturedProject[] = [
  {
    ...
    name: "Assistente JurÃ­dico IA",
    description: "App nativo Android para Maikon Caldeira OAB/MG 183712. Abre o Assistente JurÃ­dico IA em WebView nativo. EAS jÃ¡ configurado.",
    icon: "âï¸",
    color: "#6366f1",
    badge: "EAS PRONTO",
    files: [
  ...
  },
  {
    ...
    description: "API Node.js usando Groq â 100% gratuito, llama-3.3-70b. O modelo mais rÃ¡pido e inteligente sem pagar nada. Ideal para comeÃ§ar.",
    icon: "â¡",
    color: "#22c55e",
    badge: "GRÃTIS",
    files: [
  ...
  },
  {
    ...
    description: "API Node.js usando Google Gemini 2.0 Flash â gratuito com limite generoso. Crie a chave em aistudio.google.com.",
    icon: "â¨",
    color: "#3b82f6",
    badge: "GRÃTIS",
    files: [
      ...
      { path: "package.json", content: PKG_IA },
      { path: ".gitignore", content: "node_modules/\n.env\n*.log\n" },
      { path: "README.md", content: "# API com Google Gemini\n\n## Chave gratuita\n1. Acesse https://aistudio.google.com/app/apikey\n2. Crie uma chave (comeÃ§a com AIza...)\n3. Cole no .env em `GEMINI_API_KEY=`\n\n## Rodar\n```bash\nnpm install\nnpm start\n```\n\n## Testar\n```bash\ncurl -X POST http://localhost:3000/api/chat \\\n  -H \"Content-Type: application/json\" \\\n  -d '{\"messages\":[{\"role\":\"user\",\"content\":\"OlÃ¡!\"}]}'\n```\n" },
    ],
  },
  {
    id: "api-claude",
    name: "API com Claude (Anthropic)",
    description: "API Node.js usando Claude Haiku â o modelo mais inteligente do mundo. Precisa de chave paga da Anthropic.",
    icon: "ð§ ",
    color: "#f59e0b",
    badge: "TOP",
    files: [
  ...
  },
  {
    ...
    files: [
      ...
      { path: "package.json", content: PKG_IA },
      { path: ".gitignore", content: "node_modules/\n.env\n*.log\n" },
      { path: "README.md", content: "# API Multi-Modelo IA\n\nAPI que suporta Groq, Gemini, Claude e OpenAI num sÃ³ projeto.\n\n## InÃ­cio rÃ¡pido (gratuito)\n1. Crie chave Groq em https://console.groq.com/keys\n2. Cole no .env: `GROQ_API_KEY=gsk_...`\n3. `npm install && npm start`\n\n## Endpoints\n- `POST /api/chat` â envia mensagem para a IA\n- `GET /api/provedores` â lista provedores configurados\n- `GET /api/saude` â status\n\n## Escolher provedor\n```json\n{\"provedor\": \"groq\", \"messages\": [{\"role\": \"user\", \"content\": \"OlÃ¡!\"}]}\n```\nProvedor omitido = usa o primeiro configurado automaticamente.\n" },
    ],
  },
];
