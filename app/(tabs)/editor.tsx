...
const MENU_ITEMS = [
  ...
  { icon: "message-circle", label: "ð¬ Campo Livre",                  desc: "Chat sem restriÃ§Ãµes com qualquer IA",         key: "campolivre" },
  ...
  { icon: "link",           label: "Importar via Link PÃºblico",        desc: "Clonar qualquer repo pÃºblico sem token",      key: "importUrl" },
  { icon: "github",         label: "GitHub â Clonar / Enviar",        desc: "Importar ou exportar para GitHub",            key: "github" },
  { icon: "send",           label: "Publicar no RepositÃ³rio",          desc: "Enviar projeto para repositÃ³rio GitHub",      key: "publishRepo" },
  ...
  { icon: "smartphone",     label: "Gerar APK Android",               desc: "Converte o projeto em app instalÃ¡vel",        key: "apkBuilder" },
    ...
    switch (key) {
      ...
      case "importZip":
        ...
        try {
          ...
          if (!data) return;
          // usa path completo (preserva Ã¡rvore de pastas)
          createFiles(activeProject.id, data.files.map((f) => ({ path: f.path || f.name, content: f.content })));
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          Alert.alert("â ZIP importado!", `${data.files.length} arquivo(s) adicionado(s) com estrutura de pastas.`);
        } catch (e: any) {
      ...
      case "importTar":
        ...
        try {
          ...
          createFiles(activeProject.id, data.files.map((f) => ({ path: f.path || f.name, content: f.content })));
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
          Alert.alert("â TAR importado!", `${data.files.length} arquivo(s) adicionado(s) com estrutura de pastas.`);
        } catch (e: any) {
          ...
          [
            { text: "Cancelar", style: "cancel" },
            {
              ...
              onPress: () => {
                ...
                Alert.alert("â Duplicado!", `"${copy.name}" foi criado com ${activeProject.files.length} arquivo(s) com estrutura completa.`);
              },
            },
          ]
        );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingBottom: tabBarBottom }]}>
      ...
      <View style={{ flex: 1, flexDirection: "row", overflow: "hidden" }}>
        ...
        {/* ââ SIDEBAR DE ARQUIVOS (Ã¡rvore) â toggle com botÃ£o no header ââ */}
        {showSidebar && (
          <View style={{
            width: SIDEBAR_W,
            borderRightWidth: 1,
        ...
        >
          ...
          <View style={[styles.aiPanelHeader, { borderBottomColor: colors.border, paddingTop: 14 }]}>
            ...
            {/* Atalho Campo Livre â igual ao SK Code Editor */}
            <TouchableOpacity
      ...
      <Modal visible={showQuickActions} animationType="slide" presentationStyle="pageSheet" transparent onRequestClose={() => setShowQuickActions(false)}>
        ...
        <View style={{ backgroundColor: colors.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingBottom: insets.bottom + 20 }}>
          ...
          {[
            ...
            { icon: "ð", label: "Taski â Lista de Tarefas", desc: "Gerenciar to-dos do projeto", action: () => { setShowQuickActions(false); setShowTasks(true); } },
