...
const TOOLS = [
  ...
  },
  {
    ...
    desc: "Cole a URL do seu site, valida a PWA e gera APK, AAB, MSIX e IPA. Mais fÃ¡cil que Bubblewrap.",
    icon: "globe" as const,
  },
  {
    name: "PWA2APK",
    url: "https://pwa2apk.com",
    desc: "Converte PWA em APK simples. Ideal para testes rÃ¡pidos.",
    icon: "smartphone" as const,
  },
  {
    name: "GoNative.io",
    url: "https://gonative.io",
    desc: "Converte qualquer site em app nativo Android/iOS com funcionalidades extras (push, cÃ¢mera, GPS).",
    icon: "arrow-right-circle" as const,
  },
];

const REQUIRED_CHECKS: Omit<CheckItem, "status" | "detail">[] = [
  { id: "https", label: "HTTPS", description: "O site deve servir via HTTPS (obrigatÃ³rio para PWA)." },
  ...
  { id: "icons", label: "Ãcones 192x192 e 512x512", description: "O manifest deve ter Ã­cones PNG de pelo menos 192px e 512px." },
  ...
  { id: "maskable", label: "Ãcone maskable", description: "Recomendado: Ã­cone com purpose='maskable' para Ã­cone adaptativo no Android." },
  ...
  { id: "description", label: "description", description: "DescriÃ§Ã£o do app no manifest." },
];
  ...
  const validatePWA = useCallback(async () => {
    ...
    try {
      ...
      if (finalUrl.startsWith("https://") || rawUrl.startsWith("https://")) {
        ...
        update("https", "fail", "Serve via HTTP â troque para HTTPS antes de publicar.");
      }
      ...
      if (manifestMatch) {
        ...
        try {
          ...
          if (mRes.ok) {
            manifest = await mRes.json();
            manifestUrl = guess;
            update("manifest", "warn", `Manifest nÃ£o linkado no HTML mas encontrado em ${guess}`);
          } else {
            update("manifest", "fail", "Nenhum <link rel=manifest> no HTML e /manifest.json nÃ£o encontrado.");
          }
        } catch {
      ...
      if (swMatch) {
        ...
        try {
          ...
          for (const p of swUrls) {
            ...
            if (r.ok) { update("sw", "warn", `Service worker nÃ£o registrado no HTML, mas arquivo ${p} existe.`); found = true; break; }
          }
          if (!found) update("sw", "fail", "Service worker nÃ£o encontrado. Crie e registre um para cache offline.");
        } catch {
          update("sw", "fail", "NÃ£o foi possÃ­vel verificar service worker.");
        }
      }
      ...
      if (manifest) {
        ...
        if (has192 && has512) {
          update("icons", "pass", `${icons.length} Ã­cone(s) definido(s) incluindo 192x192 e 512x512.`);
        } else if (has192 || has512) {
          update("icons", "warn", `Tem ${icons.length} Ã­cone(s) mas faltam tamanhos: ${!has192 ? "192x192 " : ""}${!has512 ? "512x512" : ""}`);
        } else {
          update("icons", "fail", "Nenhum Ã­cone 192x192 ou 512x512 no manifest.");
        }
        ...
        if (m.name && m.short_name) {
          ...
          update("name", "warn", `name: "${m.name}" â falta short_name (mÃ¡x 12 chars recomendado).`);
        } else {
        ...
        if (display === "standalone" || display === "fullscreen" || display === "minimal-ui") {
          update("display", "pass", `display: "${display}" â`);
        } else if (!display) {
          update("display", "fail", "Falta 'display' no manifest. Use \"standalone\" para instalar como app.");
        } else {
          update("display", "warn", `display: "${display}" â use "standalone" ou "fullscreen" para melhor experiÃªncia.`);
        }

        // maskable
        const hasMaskable = icons.some((ic) => String(ic.purpose || "").includes("maskable"));
        update("maskable", hasMaskable ? "pass" : "warn",
          hasMaskable ? "Ãcone maskable presente â" : 'Sem Ã­cone maskable. Adicione purpose: "maskable" para Ã­cone adaptativo no Android.');
        ...
        ["icons", "name", "start_url", "display", "maskable", "theme_color", "background_color", "description"].forEach((id) =>
          update(id, "skip", "Manifest nÃ£o disponÃ­vel para verificar.")
        );
      }
      ...
      if (sc === 100) {
        setSummary("â PWA perfeita! Pronta para converter em APK com PWABuilder ou Bubblewrap.");
      } else if (sc >= 75) {
        setSummary(`â ï¸ Quase lÃ¡! Corrija: ${failItems.join(", ")} e tente converter.`);
      } else if (fail > 0) {
        setSummary(`â Faltam itens obrigatÃ³rios: ${failItems.join(", ")}. Corrija antes de converter.`);
      } else {
        setSummary("ð¶ Alguns itens recomendados estÃ£o faltando. O APK pode funcionar, mas com limitaÃ§Ãµes.");
      }
    } catch (e) {
      setError(
        `NÃ£o foi possÃ­vel acessar "${rawUrl}".\n\n` +
        `â¢ Verifique se a URL estÃ¡ correta\n` +
        `â¢ O site pode bloquear requisiÃ§Ãµes de outras origens (CORS)\n` +
        `â¢ Tente acessar pelo PWABuilder: pwabuilder.com\n\n` +
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12, backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>PWA â APK</Text>
      ...
      >
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 14, padding: 14 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: "#f1f5f9", fontSize: 15, fontWeight: "800" }}>APK Builder â Importar e compilar</Text>
              <Text style={{ color: "#94a3b8", fontSize: 12, marginTop: 2, lineHeight: 17 }}>
                Importe ZIP, TAR.GZ ou repositÃ³rio GitHub, gere um projeto Android nativo com WebView + CI automÃ¡tico no GitHub Actions.
              </Text>
            </View>
        ...
        {score !== null && (
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, alignItems: "center" }]}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>PontuaÃ§Ã£o PWA</Text>
        ...
        {/* ââ TWA: PWA â APK em 3 passos ââ */}
        <View style={{ borderRadius: 12, borderWidth: 1, borderColor: "#22c55e40", backgroundColor: "#052e16", padding: 16, gap: 12 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: "#f0fdf4", fontSize: 15, fontWeight: "800" }}>PWA â TWA â APK</Text>
              <Text style={{ color: "#86efac", fontSize: 11 }}>Trusted Web Activity â mÃ©todo mais rÃ¡pido</Text>
            </View>
          </View>

          {/* BotÃ£o principal PWABuilder */}
          <TouchableOpacity
          ...
          >
            <Text style={{ fontSize: 16 }}>ðï¸</Text>
          ...
          <Text style={{ color: "#86efac", fontSize: 12, textAlign: "center" }}>
            Cole a URL do seu site â clica "Package for Stores" â Android â baixa o APK/AAB pronto
          </Text>

          {/* Passos rÃ¡pidos */}
          {[
            { n: "1", t: "Valide a PWA acima", d: "Score mÃ­nimo 80 para PWABuilder aceitar a URL." },
            { n: "2", t: "PWABuilder â Package for Stores â Android", d: "Clique no botÃ£o acima, cole a URL, gera AAB ou APK assinado gratuitamente." },
            { n: "3", t: "Instale o APK no Android", d: "Baixe o .apk, ative 'Fontes desconhecidas' em ConfiguraÃ§Ãµes â SeguranÃ§a, abra o arquivo." },
          ...
          <View style={{ borderTopWidth: 1, borderTopColor: "#22c55e30", paddingTop: 10, gap: 6 }}>
            <Text style={{ color: "#86efac", fontSize: 12, fontWeight: "700" }}>ð² Instalar APK direto no Android</Text>
            {[
              { label: "Ativar 'Fontes desconhecidas'", url: "https://support.google.com/android/answer/9217471", icon: "unlock" as const },
              { label: "APKMirror â instalar via browser", url: "https://www.apkmirror.com", icon: "download" as const },
        ...
        {/* Como funciona */}
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Como funciona a conversÃ£o</Text>
          {[
            { n: "1", t: "Crie seu site como PWA", d: "Deve ter HTTPS, manifest.json, service worker e Ã­cones." },
            { n: "2", t: "Valide aqui", d: "Use o validador acima para checar o que estÃ¡ faltando." },
            { n: "3", t: "Use PWABuilder ou Bubblewrap", d: "Cole a URL e gere o APK/AAB com um clique." },
            { n: "4", t: "Assine e publique", d: "FaÃ§a upload do AAB na Google Play Store ou distribua o APK direto." },
          ].map((step) => (
        ...
        {/* APK Nativo via WebView â opÃ§Ã£o extra */}
        <View style={[styles.card, { backgroundColor: "#0f172a", borderColor: "#6366f144" }]}>
          ...
          <Text style={{ color: "#94a3b8", fontSize: 13, lineHeight: 19, marginBottom: 10 }}>
            Cria um app Android real que carrega seu site em WebView nativo. Ideal para hospedar um site/PWA existente como app instalÃ¡vel sem precisar da Google Play.
          </Text>
          {[
            { icon: "âï¸", title: "Template Assistente JurÃ­dico", desc: "WebView wrapper com URL configurÃ¡vel, botÃ£o voltar Android, tela de erro e FAB de configuraÃ§Ãµes. EAS jÃ¡ configurado para maikons-individual-orga2.", badge: "EAS PRONTO", action: () => Linking.openURL("devmobile://projetos") },
            { icon: "ð", title: "Template WebView GenÃ©rico", desc: "Qualquer URL vira APK. Substitua a URL no App.tsx e compile via EAS.", badge: "EXPO", action: () => Linking.openURL("https://snack.expo.dev") },
            { icon: "ð§", title: "Compilar com EAS via terminal", desc: "No terminal do app: EAS_NO_VCS=1 eas build --platform android --profile preview", badge: "TERMINAL", action: null },
          ].map((item) => (
            <TouchableOpacity
              key={item.title}
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>ð Capacidades do DevMobile</Text>
          {[
            { label: "Arquivos por projeto", value: "50.000+ arquivos", icon: "file", color: "#10b981" },
            { label: "Formatos de importaÃ§Ã£o", value: "ZIP, TAR, TAZ, TGZ, Git Clone", icon: "download", color: "#3b82f6" },
            { label: "Formatos de exportaÃ§Ã£o", value: "ZIP, compartilhamento direto", icon: "upload", color: "#f59e0b" },
            { label: "CompilaÃ§Ã£o APK", value: "EAS Build (Expo) + PWAâAPK", icon: "cpu", color: "#8b5cf6" },
