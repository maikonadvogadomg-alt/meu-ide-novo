  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: bottomPadding + 20 }}>
        ...
        {/* EAS / Compilar APK */}
        <SectionHeader title="EAS BUILD â COMPILAR APK" colors={colors} />
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={{ padding: 14, gap: 14 }}>
            <Text style={{ color: colors.mutedForeground, fontSize: 12, lineHeight: 18 }}>
              Configure sua conta Expo para compilar APKs. VocÃª pode usar qualquer conta â nÃ£o precisa ser a conta maikon1.
            </Text>

            {/* Conta Expo */}
            <View style={{ gap: 6 }}>
              <Text style={{ color: colors.foreground, fontWeight: "600", fontSize: 13 }}>
                ð¤ Conta Expo (username)
              </Text>
              {expoEditing ? (
                <View style={{ gap: 8 }}>
                  <TextInput
                  ...
                  <Text style={{ color: colors.foreground, fontWeight: "600", fontSize: 13, marginTop: 4 }}>
                    ð§ Slug do Projeto EAS (para DevMobile)
                  </Text>
                  <TextInput
                    value={easSlugDraft}
                  ...
                  <View style={{ flexDirection: "row", gap: 8, marginTop: 4 }}>
                    <TouchableOpacity
                      onPress={() => {
                        ...
                        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
                        Alert.alert("â Salvo!", `Conta: ${expoAccountDraft.trim()}\nSlug: ${easSlugDraft.trim() || "app-ide"}\n\nAgora o APK serÃ¡ compilado com esta conta.`);
                      }}
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {dbConfigs.map((d) => (
            <SettingRow
              ...
              label={d.name}
              sublabel={`${
                d.provider === "neon" ? "â¡ Neon PostgreSQL"
                : d.provider === "supabase" ? "ð¢ Supabase"
                : d.provider === "postgres" ? "ð PostgreSQL"
                : d.provider === "mysql" ? "ð¬ MySQL"
                : d.provider === "mongodb" ? "ð MongoDB"
                : d.provider === "sqlite" ? "ð¦ SQLite"
                : d.provider === "turso" ? "ð¦ Turso"
                : d.provider === "redis" ? "ð´ Redis"
                : d.provider === "firebase" ? "ð¥ Firebase"
                : d.provider === "planetscale" ? "ðª PlanetScale"
                : d.provider === "railway" ? "ð Railway"
                : d.provider
              } Â· IA ativada`}
              right={
                <View style={{ flexDirection: "row", gap: 10, alignItems: "center" }}>
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border, padding: 12 }]}>
          ...
          {settings.geminiDirectKey ? (
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 8 }}>
              ...
              <Text style={{ color: colors.mutedForeground, fontSize: 12 }}>Sem chave â depende do servidor externo</Text>
            </View>
          )}
        </View>
          ...
          <View style={{
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              <View style={{ flex: 1, gap: 2 }}>
                <Text style={{ color: colors.foreground, fontWeight: "700", fontSize: 15 }}>
                  {settings.serverEnabled ? "ð¢ Servidor ligado" : "â« Servidor desligado"}
                </Text>
            ...
            {!settings.serverEnabled && (
              <View style={{ backgroundColor: "#1a1a0a", borderRadius: 8, padding: 10, borderWidth: 1, borderColor: "#ca8a0444", gap: 4 }}>
                <Text style={{ color: "#fbbf24", fontWeight: "700", fontSize: 12 }}>â ï¸ Servidor desligado</Text>
                <Text style={{ color: "#fde68a", fontSize: 11, lineHeight: 16 }}>
                  {"â¢ IA: use Groq (grÃ¡tis), Gemini, OpenAI, etc. com sua chave\nâ¢ Terminal: nÃ£o disponÃ­vel (precisa de servidor)\nâ¢ VS Code: abre direto no vscode.dev (sem servidor)\nâ¢ GitHub: funciona normalmente"}
                </Text>
              </View>
            )}
            ...
            {settings.serverEnabled && (
              <View style={{ backgroundColor: "#0d2b0d", borderRadius: 8, padding: 10, borderWidth: 1, borderColor: "#22c55e44", gap: 4 }}>
                <Text style={{ color: "#4ade80", fontWeight: "700", fontSize: 12 }}>â Servidor ativo</Text>
                <Text style={{ color: "#86efac", fontSize: 11, lineHeight: 16 }}>
                  {"â¢ IA Cortesia Gemini funciona via servidor\nâ¢ Terminal Linux disponÃ­vel\nâ¢ VS Code Server disponÃ­vel (se configurado)\nâ¢ Termux ou VPS detectado automaticamente"}
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={{ padding: 14, gap: 10 }}>
            ...
            <View style={{ gap: 6 }}>
              ...
              <Text style={{ color: colors.mutedForeground, fontSize: 11, lineHeight: 16 }}>
                Porta onde o servidor roda no seu celular via Termux. PadrÃ£o: 8080. Mude se precisar de outra porta.
              </Text>
              {termuxPortEditing ? (
                <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
                  ...
                  >
                    <Text style={{ color: colors.mutedForeground, fontSize: 13 }}>â</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={{ padding: 14, gap: 12 }}>
            ...
            <Text style={{ color: colors.mutedForeground, fontSize: 12, lineHeight: 18 }}>
              Seu celular vira o servidor. Sem internet, sem servidor externo, sem nuvem. Terminal real, VS Code real, plugins reais â tudo rodando no seu Android via Termux.
            </Text>
        ...
        <SectionHeader title="TERMINAL SEM SERVIDOR EXTERNO â MICROSOFT / QUALQUER VPS" colors={colors} />
        ...
        <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <SettingRow colors={colors} label="DevMobile IDE" sublabel="v1.9.5 â Gemini direto do celular + Terminal offline" />
        ...
        <View style={[styles.modal, { backgroundColor: colors.background }]}>
          <View style={[styles.modalHeader, { borderBottomColor: colors.border, paddingTop: 48 }]}>
            <View>
              ...
              <Text style={{ color: colors.mutedForeground, fontSize: 12, marginTop: 2 }}>
                A IA recebe a conexÃ£o e gera cÃ³digo automaticamente
              </Text>
            </View>
          ...
          <ScrollView contentContainerStyle={[styles.modalBody, { paddingBottom: 60 }]} keyboardShouldPersistTaps="handled">
            ...
            {/* Tipo de banco */}
            <Text style={[styles.label, { color: colors.mutedForeground }]}>TIPO DE BANCO / SERVIÃO</Text>
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
              {([
                { id: "neon",        label: "Neon",         emoji: "â¡", color: "#00d4aa" },
                { id: "supabase",    label: "Supabase",     emoji: "ð¢", color: "#3ecf8e" },
                { id: "postgres",    label: "PostgreSQL",   emoji: "ð", color: "#336791" },
                { id: "mysql",       label: "MySQL",        emoji: "ð¬", color: "#f59e0b" },
                { id: "mongodb",     label: "MongoDB",      emoji: "ð", color: "#4DB33D" },
                { id: "sqlite",      label: "SQLite",       emoji: "ð¦", color: "#60a5fa" },
                { id: "turso",       label: "Turso",        emoji: "ð¦", color: "#a78bfa" },
                { id: "redis",       label: "Redis",        emoji: "ð´", color: "#ef4444" },
                { id: "firebase",    label: "Firebase",     emoji: "ð¥", color: "#ff8a00" },
                { id: "planetscale", label: "PlanetScale",  emoji: "ðª", color: "#e879f9" },
                { id: "railway",     label: "Railway",      emoji: "ð", color: "#a855f7" },
            ...
            {/* InstruÃ§Ãµes do banco selecionado */}
            <View style={[styles.hint, { backgroundColor: colors.secondary, borderColor: colors.border, marginBottom: 12 }]}>
              ...
              <Text style={[styles.hintText, { color: colors.mutedForeground, lineHeight: 18 }]}>
                {dbProvider === "neon"
                  ? "Neon (grÃ¡tis): console.neon.tech â crie projeto â Connection string â Node.js â copie a URL completa"
                  : dbProvider === "supabase"
                  ? "Supabase: supabase.com â projeto â Settings â Database â Connection string (URI) â copie"
                  : dbProvider === "postgres"
                  ...
                  : dbProvider === "mongodb"
                  ? "MongoDB Atlas: cluster â Connect â Drivers â copie a connection string (mongodb+srv://...)"
                  : dbProvider === "sqlite"
                  ? "SQLite: caminho local do arquivo .db (ex: /data/meuapp.db) ou :memory: para memÃ³ria"
                  : dbProvider === "turso"
                  ? "Turso (SQLite na nuvem): turso.tech â banco â Connect â copie a URL libsql:// e o token de autenticaÃ§Ã£o"
                  : dbProvider === "redis"
                  ? "Redis: Upstash (grÃ¡tis), Railway. Formato: redis://default:senha@host:6379 ou rediss://..."
                  : dbProvider === "firebase"
                  ? "Firebase: console.firebase.google.com â projeto â ConfiguraÃ§Ãµes â serviceAccountKey.json â copie o JSON completo"
                  : dbProvider === "planetscale"
                  ? "PlanetScale: app.planetscale.com â banco â Connect â Node.js â copie a DATABASE_URL"
                  : "Railway: railway.app â projeto â banco â Variables â DATABASE_URL â copie"}
              </Text>
            </View>
            ...
            <View style={[styles.hint, { backgroundColor: "#00d4aa12", borderColor: "#00d4aa44", marginBottom: 16 }]}>
              ...
              <Text style={[styles.hintText, { color: "#00d4aa", lineHeight: 18 }]}>
                {`A IA receberÃ¡ a conexÃ£o e poderÃ¡:\nâ¢ Criar tabelas, migrations, CRUD completo\nâ¢ Escrever cÃ³digo de conexÃ£o (Prisma, Drizzle, mongoose, etc)\nâ¢ Fazer consultas SQL / NoSQL por voz`}
              </Text>
            </View>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 10, marginBottom: 4 }}>
              <Text style={[styles.label, { color: colors.mutedForeground, marginTop: 0, marginBottom: 0 }]}>
                {dbProvider === "firebase" ? "SERVICE ACCOUNT JSON / CREDENCIAL" : "STRING DE CONEXÃO / URL"}
              </Text>
              <TouchableOpacity
                onPress={async () => {
                  try {
            ...
            {dbConnStr.trim().length > 0 && (
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 4 }}>
                ...
                <Text style={{ color: "#22c55e", fontSize: 12 }}>
                  {dbConnStr.trim().length} caracteres â pronto para salvar
                </Text>
              </View>
            )}
