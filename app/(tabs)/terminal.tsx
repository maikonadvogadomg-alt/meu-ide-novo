...
export default function TerminalScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.terminalBg, paddingBottom: tabBottom }]}>
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.headerSub, { color: hasServer ? "#22c55e" : "#f59e0b" }]}>
            {hasServer ? "ð¢ Servidor online â bash, node, python, git, npm" : "ð¡ Sem servidor â use os editores na nuvem abaixo"}
          </Text>
        </View>
        <TouchableOpacity
      ...
      {/* Fluxo recomendado â aparece quando nÃ£o hÃ¡ servidor */}
      {!hasServer && (
        <View style={styles.workflowCard}>
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={styles.workflowTitle}>ð Terminal na Nuvem â Sem Instalar Nada</Text>
          ...
          {showWorkflowGuide && (
            <View style={styles.workflowBody}>
              <Text style={styles.workflowStep}>
                {"1ï¸â£  Empurre seu projeto para o GitHub (aba Projetos â GitHub)"}
              </Text>
              <Text style={styles.workflowStep}>
                {"2ï¸â£  Abra no editor abaixo â tem terminal Linux real, npm install, node, python"}
              </Text>
              <Text style={styles.workflowStep}>
                {"3ï¸â£  Trabalhe lÃ¡: instale pacotes, rode cÃ³digo, edite arquivos"}
              </Text>
              <Text style={styles.workflowStep}>
                {"4ï¸â£  FaÃ§a push/pull e seu projeto fica sincronizado"}
              </Text>
            </View>
          )}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cloudBtns}>
            <TouchableOpacity onPress={openStackBlitz} style={[styles.cloudBtn, { backgroundColor: "#1389fd22", borderColor: "#1389fd66" }]}>
              <Text style={styles.cloudBtnEmoji}>â¡</Text>
            ...
            <TouchableOpacity onPress={openGitpod} style={[styles.cloudBtn, { backgroundColor: "#ff8a0022", borderColor: "#ff8a0066" }]}>
              <Text style={styles.cloudBtnEmoji}>ð </Text>
            ...
            <TouchableOpacity onPress={openCodespaces} style={[styles.cloudBtn, { backgroundColor: "#60a5fa22", borderColor: "#60a5fa66" }]}>
              ...
              <Text style={[styles.cloudBtnSub, { color: "#60a5fa99" }]}>60h grÃ¡tis/mÃªs</Text>
            </TouchableOpacity>
            <TouchableOpacity
          ...
          {!ghPath && (
            <Text style={styles.workflowHint}>
              ð¡ Dica: Vincule o projeto ao GitHub primeiro para abrir direto no repo
            </Text>
          )}
        </View>
      )}
