...
export default function TreeScreen() {
  ...
  const dispararBuild = async () => {
    if (!easToken.trim()) {
      Alert.alert("Token obrigatÃ³rio", "Cole seu token EAS antes de disparar.");
      return;
    }
    setEasBuilding(true);
    setEasMsg("ð Disparando build...");
    try {
      ...
      if (json?.errors?.length) {
        setEasMsg(`â ${json.errors[0].message}`);
      } else if (json?.data?.build?.createBuild?.id) {
        const id = json.data.build.createBuild.id;
        setEasMsg(`â Build disparado!\nID: ${id}\n\nexpo.dev â Projects â ${slug} â Builds`);
      } else {
        setEasMsg("â ï¸ Resposta inesperada. Verifique o token e o Package ID no Expo.");
      }
    } catch (e) {
      setEasMsg(`â Erro de conexÃ£o: ${String(e)}`);
    } finally {
        ...
        <View style={{
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            {easToken.trim() ? (
              <View style={{ marginLeft: "auto", backgroundColor: "#10b98122", borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                <Text style={{ color: "#10b981", fontSize: 10, fontWeight: "700" }}>token salvo â</Text>
              </View>
            ) : null}
          </View>

          <Text style={{ color: colors.mutedForeground, fontSize: 11, lineHeight: 17 }}>
            Cole seu token EAS abaixo e toque em DISPARAR. O build roda nos servidores do Expo e gera o APK para vocÃª baixar.
          </Text>
          ...
          >
            ...
            <Text style={{ color: "#fff", fontSize: 15, fontWeight: "800" }}>
              {easBuilding ? "Disparando..." : "ð DISPARAR"}
            </Text>
          </TouchableOpacity>
          ...
          {easMsg ? (
            <View style={{
              padding: 12,
              borderRadius: 8,
              backgroundColor: easMsg.startsWith("â") ? "#052e16" : easMsg.startsWith("ð") ? colors.secondary : "#450a0a",
              borderWidth: 1,
              borderColor: easMsg.startsWith("â") ? "#10b98133" : easMsg.startsWith("ð") ? colors.border : "#ef444433",
            }}>
              <Text style={{
                color: easMsg.startsWith("â") ? "#4ade80" : easMsg.startsWith("ð") ? colors.mutedForeground : "#f87171",
                fontSize: 12,
                lineHeight: 18,
                fontFamily: "monospace",
              }}>
