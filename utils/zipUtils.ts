...
// ââ Pastas que sÃ£o lixo de build/deps â ignorar ao importar âââââââââââââââââ
const SKIP_DIRS = [
...
// ââ LÃª arquivo como Uint8Array â SEM base64, evita truncamento do Hermes âââââ
async function readFileAsBytes(uri: string): Promise<Uint8Array> {
  // Attempt 1: expo-file-system/next File.bytes() â mais confiÃ¡vel no Android
  try {
  ...
  throw new Error("NÃ£o foi possÃ­vel ler o arquivo. Tente novamente.");
}

// Mantida apenas para arquivos binÃ¡rios individuais (imagem/PDF)
async function readFileAsBase64(uri: string): Promise<string> {
  const bytes = await readFileAsBytes(uri);
  // Converte Uint8Array â base64 em chunks para nÃ£o travar o Hermes
  const CHUNK = 8192;
  let binary = "";
...
// ââ Remove pasta raiz Ãºnica do ZIP/TAR ââââââââââââââââââââââââââââââââââââââââ
...
// ââ TAR parser ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// CORREÃÃO PRINCIPAL: lÃª como Uint8Array direto (nÃ£o mais base64 string)
// O Hermes (motor JS do Android) trunca strings > ~16MB â base64 corrompido
// â apenas 10 de 200 arquivos apareciam. Agora lÃª binÃ¡rio direto: sem truncamento.
...
export async function importZip(
  ...
  // â CORREÃÃO: lÃª como bytes binÃ¡rios â sem base64, sem truncamento do Hermes
  let fileBytes: Uint8Array;
  try {
    fileBytes = await readFileAsBytes(asset.uri);
  } catch (e: any) {
    throw new Error(`NÃ£o foi possÃ­vel ler o arquivo: ${e?.message}`);
  }
  ...
  // Coleta entradas vÃ¡lidas, filtrando node_modules e lixo de build
  ...
  if (Object.keys(rawFiles).length === 0) {
    throw new Error("O ZIP estÃ¡ vazio ou todos os arquivos sÃ£o binÃ¡rios/node_modules.");
  }
...
// CORREÃÃO: mesmo fix â lÃª como bytes direto, sem base64 intermediÃ¡rio
...
export async function importTar(
  ...
  // â CORREÃÃO: bytes direto, sem base64
  ...
  const projectFiles: ProjectFile[] = tarFiles
    ...
    .map((f, idx) => {
      ...
      if (isBinaryFile(f.name)) {
        // Chunks de 8192 para nÃ£o travar o Hermes
        const CHUNK = 8192;
        let binary = "";
  ...
  if (projectFiles.length === 0) {
    throw new Error("O TAR estÃ¡ vazio ou corrompido.");
  }
  ...
  return {
    name: projectName,
    description: `Importado de ${filename} â ${projectFiles.length} arquivo(s)`,
    files: projectFiles,
  };
}
